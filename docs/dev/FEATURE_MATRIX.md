...
> 2026-07-13 wirklich gebaut wurde — mehrere der reinen Kerne leben weiter (`RPPGConditioning`
> ist heute der rPPG-Pfad). **Warum der Marker seine Bytes wert ist:** der Block ist
> PRESKRIPTIV. „open gaps“ + nummerierter Rückstand liest sich als Auftrag, und eine Sitzung,
> die ihn abarbeitet, baut genau die Flächen zurück, die der Founder zweimal entfernen ließ.
>
...
> - **EchoelVis is LIVE** — `Views/MetalBioView.swift` is the live full-screen bio visual (HR→pulse ≤2.5 Hz WCAG, sounding tone→hue, breath→spread, coherence→sharpness, honours Reduce Motion — #1116 corrected "coherence→hue", which no shipped build of this renderer implemented; `BioVisualParams.hue` has no consumer). NOT dormant/deprecated. **EchoelSeq = 19 offered genres** (corrected 2026-08-06 to 16, 2026-09-04 to 19 with #983 S3–S5 `deepTech`/`darkMinimal`/`psyProgHouse`; #254 added eight and every claim of "8" went stale — the "23"/"12" figures before that were both wrong too. Count with `MusicStyle.offered.count`, never from memory). **sACN unicast is LIVE** beside Art-Net.
...
> - ⛔ **TWO OF THOSE CORES NO LONGER EXIST, and this is a dated history block, so the line above stays as written** (#1302, founder 2026-09-12): `Studio/VocoderCore.swift` and `Studio/FeedbackGuard.swift` are **deleted as files** (`git ls-files "*VocoderCore.swift"` → 0). They guarded and vocoded a microphone path that is gone. **The reason this marker is worth its bytes even inside history:** the struck line is not merely dated, it is PRESCRIPTIVE — "built but NOT yet wired" is an invitation to wire, and a session reading it would go looking for a flagship vocoder and find nothing. `BioVisualParams` is now WIRED (`MetalBioView`); `BioModulation` and `CloudSync` are still unwired and still present.
> - ⛔ **ZURÜCKGENOMMEN (#1343, 2026-09-16) — Wortlaut absichtlich erhalten:** *„Clips/Arrangement: domain (`Clip`/`Arrangement`/`ArrangementPlayer`/`LaunchQuantizer` + stores) + tests EXIST; the **UI is the open gap** (#1).“*
>   **Das ist die teuerste Zeile dieser Datei, und der Wortlaut bleibt stehen, weil er
>   datierte Historie ist.** Sie nennt eine gestrichene Fläche den
>   **„open gap (#1)“** — also den höchstplatzierten offenen Posten des Registers. Die
>   Arrangement-/Clip-Oberfläche ist die am ausdrücklichsten gestrichene Sache dieses Repos:
>   `PRODUCT_DEFINITION.md`s GRENZE schneidet sie namentlich, `#121` Slice 4 hat `ClipView`
>   (`807dc0d`) und `ArrangeTimelineView` (`eb58e7a`) gelöscht. `LaunchQuantizer` existiert
>   nicht mehr (`git grep -c "enum LaunchQuantizer\|struct LaunchQuantizer" -- Sources` → 0);
>   `Clip`, `Arrangement`, `ArrangementPlayer`, `ClipStore`, `ArrangementStore` und
>   `AutomationLane` leben weiter, aber als MODELL ohne Tür — die #527-Lage, nicht ein Auftrag.
>   **Ein „open gap (#1)“ wird gelesen wie eine Priorität, nicht wie ein Datum.**
...
- ⛔ **"Live (voice timbre, #592a/#593)" STOOD HERE AND ITS CAPTURE HALF IS DELETED** (#1302, founder 2026-09-12, "Face und Audio Input komplett entfernen"). It described a held tone captured in the Sound panel's "Voice timbre" row via `Studio/VoiceCaptureController` → `VoiceCaptureEngine` → `VoiceAnalyzer`. **All three are gone as FILES** (`git ls-files "*VoiceCaptureEngine.swift"` → 0); there is no microphone, so nothing can capture a tone. ⚠️ **The PATCH half survives and is not a leftover:** `SynthPatch.voiceProfileTaps` (~64 floats, max-normalized) plus its label and blend are still decoded and still applied by `EchoelDDSP` — a patch saved by an older build carries them and SOUNDS (#95/#527). CLAUDE.md names that split explicitly. So: the row is gone, the spectrum a stored patch carries is not. ⚠️ The struck entry closed with *"the reference document a session reads to decide what is live did not know the feature existed"* — this page then spent four days telling that same session the feature was LIVE after it was deleted. **A register fails in both directions, and the second is worse:** an absence gets discovered by whoever looks for the feature, a false present tense gets planned from.
- ⛔ **"modal / cellular" stood in that Live line and made no sound (#796) — and HALF of that retraction has itself expired (#1410, 2026-09-21).** `EchoelModalBank` is unchanged: zero production instantiators, its only caller was the drum voice removed by #167, and it stays OUT of any Live line. **`EchoelCellular` is NOT unchanged** — #1385 brought the AUv3 target back and its `texture` voice instantiates it, so it has ONE production site and it sounds **in the plug-in**. It still stays out of *this* Live line, because this section describes the iPhone app and no voice there instantiates it; say "plug-in only", never "makes no sound". ⭐ **The bullet directly above this one closes with "a register fails in both directions, and the second is worse" — and this bullet was the first direction, aging DOWNWARD: a capability that grew while its retraction stood still.** An under-claim carries no App-Store risk, which is exactly why nobody re-reads it; its cost is that the next cycle plans around a module it believes is dead (here: the ~20-percentage-point AUv3 DSP finding, #1386). Measure, do not quote: `git grep -n "EchoelModalBank(\|EchoelCellular(" -- Sources`. ⚠️ The grep on the bare NAME is polluted by prose — many files under `Sources/` mention them in comments; the grep that measures the thing is the one on `Name(`, and `EchoelCellular.swift` itself quotes that recipe, so strip comments before counting.
...
- **Live:** master bus, a 1024-pt FFT window on the **master OUTPUT** (⛔ this read "mic FFT" and there is no microphone since #1302 — measured: the only two `installTap`s in `Audio/` sit on `meterNode` and `masterMixer`, and the ring they fill is documented "for the immersive FFT visual". The word that was wrong is the one that decides the direction, and it sat ONE LINE below its own #1302 tombstone), 30 s stereo pre-roll ring (`.caf`, taps the own output too), LUFS-normalized WAV/AAC export.
- **Roadmap:** console UI, FLAC/ALAC, stem export. ⛔ `Audio/MultiTrackRecorder.swift` (skeleton) stood at the head of this list and is **deleted** (#1302) — a roadmap row pointing at a removed file reads as "almost there" when the truth is "would be written from scratch, and the founder took the microphone out".
...
- **Character params — LIVE (2026-06-17):** `MoodProfile` now 8 dims (… + **virtuosity, syncopation, humanize**), all consumed in the lead generator. **Die angebotenen Genres** (`MusicStyle.offered`) sind eine TEILMENGE der deklarierten — die Kuratierung ist nach Gehör, und ein weiteres anzubieten ist eine Hör-Entscheidung. **Beide Zahlen stehen hier bewusst NICHT als Literal (#818): messen mit `python3 scripts/genre-prebatch.py <leere-kandidatenliste.json>`**, das druckt `SHIPPED: N genres, M offered, P patches` aus dem Quelltext. ⛔ **Hier standen „19 angeboten“ und „der Enum hält 36 Fälle“, und der Befund ist schärfer als „zwei veraltete Zahlen“ (#1343, 2026-09-16): die DIFFERENZ blieb richtig. 36−19 = 17, und heute ist 50−33 ebenfalls **17** — die Zeile behauptete also einen Rest, der weiterhin stimmt, während BEIDE Operanden um 14 danebenlagen. Genau die Form, vor der `.claude/rules/context.md` §2 warnt: zwei Zahlen, die zueinander passen, sind kein Mengenvergleich — und die stimmende Differenz ist das, was ein Prüfer als Bestätigung liest und weiterblättert. ⛔ This said "nine older ones": 36 − 19 = 17 (33 − 16 before #983 S3–S5 added three genres to both sides), and it was 17 at the 2026-07-24 curation too (25 declared, 8 offered). Nothing in the tree counts to nine).
...
- **Bio-generative composer ("your heartbeat composes"):** `Sequencer/MusicalKey.swift` (set your own key/scale, **57 scales** — `Scale.Family.allCases.flatMap(\.scales)`, which is what the picker iterates) + `Sequencer/MusicStyle.swift` (die angebotenen Genres — alle harmonisch/melodisch, seit die Drums entfernt sind. ⛔ **Hier standen eine Zahl UND die vollständige Namensliste, beide veraltet (#1343)**; die Liste war eine zweite Abschrift von `MusicStyle.offered` (#416) und ist gelöscht statt nachgeführt. Messen: `python3 scripts/genre-prebatch.py <leere-kandidatenliste.json>`. ⚠️ **Die Zahl steht auf sieben nutzersichtbaren Flächen, und die hält ein Wächter aktuell** (`WebsitePagesAreFindableAndHonestTests`) — dieses Register war in keinem, also driftete genau die Datei, die sich „Truth-source for status“ nennt, während die Website mitzog) + `Sequencer/BioComposer.swift` (bio → in-key melody + heartbeat rhythm + tempo, SplitMix64-seeded → reproducible) + `Sequencer/GenrePatches.swift` (per-genre synth timbre), Two modes: **Studio** (BPM-locked, for Ableton/FL handoff) and **Flow** (sync-free, follows the heart — for meditation). Each take gets its genre timbre + genre/character FX space. Generated melody is written as MIDI (real durations + velocity) and the door is LIVE — #188 put `exportMIDI()` back into the existing export slot (`EchoelStudioView.swift:6579`); the live store description claims it. Surfaced as **"Generate from Body"** in `Studio/EchoelStudioView.swift`; the `Studio/ComposeView.swift` this line used to cite no longer exists.
...
- **TestFlight acceptance — ⛔ DIE ERSTE HÄLFTE IST UNPRÜFBAR (#1343, 2026-09-16):** ~~`BeatTab` plays a pattern at the set BPM; accent louder; swing audible; a loaded sample replaces a pad and survives relaunch~~ — `BeatTab` wurde **nie gebaut** (der `StudioRoot`-TabView-Plan ist in `CLAUDE.md` als nie umgesetzt vermerkt), und Pads, Drums und die Sample-Bibliothek sind mit `#166`/`#167` gelöscht. **Ein Abnahmekriterium, das auf dem Gerät nicht bestehen KANN, ist schlimmer als keins**: wer es abarbeitet, meldet einen Fehlschlag gegen einen Maßstab, den der Founder selbst entfernt hat. Gültig bleibt: **"Generate from Body" writes an in-key melody in the chosen genre + key, applies the genre/character FX, and exports a stamped MIDI filename; Studio locks the BPM, Flow follows the heart.**
...
- **Code (live):** `Views/MetalBioView.swift` (Metal bio visual, inline-compiled shader, AdaptiveQuality FPS/detail tiers, flash-safe ≤3 Hz, Reduce Motion) inside `Studio/FloatingVisualWindow.swift` — das schwebende Fenster, umgeschaltet aus dem `WorkspaceView`-Kopf; seine Größen sind `small/medium/large/fullscreen` (`FloatingVisualWindow.WindowSize`). ⛔ **„with in-fullscreen VJ controls + palette“ stand hier und ist mit #1069 gelöscht (#1343, 2026-09-16):** `visualVJOverlay` und das Vollbild-FELD (`.fullScreenCover(isPresented: $showVisual)`) sind weg — messen — und zwar die SACHE, nicht den NAMEN: `git grep -c "var visualVJOverlay" -- Sources` → **0**. ⚠️ Der naheliegende `git grep -c "visualVJOverlay"` liefert **5** und wäre als Beleg das Gegenteil: das sind die Grabstein-Kommentare, die die Löschung festhalten (die `EchoelModalBank`-Lehre — jeder Kommentar ÜBER die Sache verfälscht ihren eigenen Beleg). ⚠️ **Die Unterscheidung ist nicht kosmetisch und eine unachtsame Reparatur fällt in die andere Richtung:** „Vollbild“ als GRÖSSE des schwebenden Fensters lebt weiter; gestorben ist die ZWEITE, getrennte Vollbild-Chrome. Wer den Satz ganz streicht, nimmt eine Fläche zurück, die es gibt.
...
- **TestFlight acceptance:** header monitor toggles the floating visual; it reacts to bio; the window resizes to `fullscreen`. ⛔ **„+ record work“ stand hier und ist mit #1304 gegenstandslos (#1343, 2026-09-16):** es gibt keine Aufnahme und kein MP4 — die Zeile zwei höher sagt das selbst, seit `VisualRecorder`/`VideoMuxer` gelöscht sind. **Zweites unprüfbares Abnahmekriterium derselben Datei**; das erste ist `BeatTab` weiter oben. Ein Register, das seinen eigenen ⛔-Vermerk zwei Zeilen später im Abnahmekriterium wieder aufhebt, ist die Form, die eine Geräte-Sitzung kostet.
...
- **Live:** OSC 1.0 over UDP — 5 continuous `/echoelmusic/bio/*` (`/bio/motion` is the 6th and is NOT sent in this build, #215: no motion sensor) + 6 discrete `/echoelmusic/bio/event/*` + `/echoelmusic/mod/*` (modulation), default `localhost:8000`. **Both bio paths carry provenance** via `/echoelmusic/bio/synthetic` (1 = the built-in demo generator, 0 = a real body): on the continuous batch it rides every tick that sends at least one value (#639); on the discrete events it is sent immediately before the event it describes and again only when the origin CHANGES, latched across polls (#785). Latch it as state — UDP does not preserve order. **ADM-OSC** immersive object output (`/adm/obj/{n}/{azim|elev|dist}` + `/gain`, bio→object) into FletcherMachine/L-ISA/d&b — opt-in from the Sync tab. MIDI 1.0/2.0 input (⛔ **not MPE in** — #548/#770: the parser reads MPE traffic but tells no zones apart, and the consumer never reads `event.channel`, so no member channel exists to tell apart. The clause "`MIDIEventParse` has no Channel Pressure case at all … a single `break` for slide, air-CC and channel pressure" stood here until #939 parsed Press and #942 sounded Slide; only `.airCC` still breaks, and it is not an MPE dimension. The retraction is unchanged because it rests on ZONES, not on a count. **MPE OUT is real and switchable** since #713).
