...
| `/echoelmusic/bio/heart/rmssd` · `/sdnn` · `/pnn50` | ms / ms / % | HRV detail (trusted source) — same rule: a pulse plus a real value. **OFF by default since #1292**: these three are the clinical statistics, opt in under Routing → "Send clinical HRV detail". Everything else in this table flows unswitched. |
