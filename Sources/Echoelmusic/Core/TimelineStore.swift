...
public final class TimelineStore {
    ...
    private func apply(_ step: HistoryStep) -> HistoryStep? {
        ...
        case .automation(let lanes):
            ...
            persist()
            return inverse
        case .clipSource(let clipID, let mediaRef, let seconds, let assetID, let record, let clips):
            guard let clip = clips.clip(id: clipID), clip.kind == .audio,
                  let current = clip.mediaRef, !current.isEmpty else { return nil }
            // The record's binding NOW is the inverse's. A record that has since left the registry
            // takes no part: the clip still changes back, its source has nothing to move.
            let currentRecord = record.flatMap { moved in
                moved.store.record(id: moved.recordID).map {
                    MediaAssetStore.Rebinding(store: moved.store, recordID: moved.recordID,
                                              fileName: $0.fileName)
                }
            }
            ...
            if let record, currentRecord != nil {
