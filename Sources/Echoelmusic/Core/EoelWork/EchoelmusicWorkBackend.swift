//
//  EchoelmusicWorkBackend.swift
//  Echoelmusic
//
//  Created: 2025-11-24
//  Copyright © 2025 Echoelmusic. All rights reserved.
//
//  Complete backend implementation for EchoelmusicWork multi-industry gig platform
...
/// Complete EchoelmusicWork backend implementation with Firebase
@MainActor
final class EchoelmusicWorkBackend: ObservableObject {
    static let shared = EchoelmusicWorkBackend()
    ...
    @Published var currentUser: EchoelmusicWorkUser?
    ...
    /// Sign up new user
    func signUp(email: String, password: String, profile: UserProfile) async throws -> EchoelmusicWorkUser {
        isLoading = true
    ...
    private func saveUserProfile(_ user: EchoelmusicWorkUser) async throws {
        let data: [String: Any] = [
            "email": user.email,
            "profile": [
    ...
    private func loadUserProfile(uid: String) async throws {
        ...
        guard let data = document.data() else {
            throw EchoelmusicWorkError.userNotFound
        }
    ...
    func updateProfile(_ profile: UserProfile) async throws {
        guard var user = currentUser else { throw EchoelmusicWorkError.notAuthenticated }
    ...
    func postGig(_ gig: Gig) async throws -> String {
        guard let user = currentUser else { throw EchoelmusicWorkError.notAuthenticated }
    ...
    func searchGigs(
        industry: EchoelmusicWorkManager.Industry? = nil,
    ...
    func findMatchingGigs(for user: EchoelmusicWorkUser) async throws -> [Gig] {
    ...
    private func getGig(gigId: String) async throws -> Gig {
        ...
        guard let data = document.data() else { throw EchoelmusicWorkError.gigNotFound }
    ...
    func processPayment(amount: Double, contractId: String) async throws -> String {
        ...
        guard let paymentIntentId = result.data as? String else {
            throw EchoelmusicWorkError.paymentFailed
        }
    ...
    private func decodeUser(from data: [String: Any], uid: String) throws -> EchoelmusicWorkUser {
        guard let email = data["email"] as? String,
            ...
            throw EchoelmusicWorkError.decodingFailed
        }

        let industries = industriesRaw.compactMap { EchoelmusicWorkManager.Industry(rawValue: $0) }
    ...
    private func decodeGig(from data: [String: Any], id: String) throws -> Gig {
        guard let title = data["title"] as? String,
              ...
              let industry = EchoelmusicWorkManager.Industry(rawValue: industryRaw),
...
enum EchoelmusicWorkError: LocalizedError {
