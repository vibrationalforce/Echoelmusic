...
@MainActor
final class EchoelmusicWorkManager: ObservableObject {
    static let shared = EchoelmusicWorkManager()

    // MARK: - Published State

    @Published private(set) var currentUser: EchoelmusicWorkUser?
    ...
    func initialize() async throws {
        // Initialize EchoelmusicWork services
        print("✅ EchoelmusicWork initialized")
    }

    // MARK: - User Management

    func signUp(profile: UserProfile) async throws -> EchoelmusicWorkUser {
...
struct EchoelmusicWorkUser: Identifiable {
    let id: UUID
    var profile: UserProfile
    var industries: [EchoelmusicWorkManager.Industry]
...
struct Gig: Identifiable {
    ...
    var industry: EchoelmusicWorkManager.Industry
    var budget: Double
    var urgency: Urgency
    var location: CLLocation
...
struct Contract: Identifiable {
    let id: UUID
    var gig: Gig
    var provider: EchoelmusicWorkUser
    var status: Status
    var createdAt: Date
...
enum SubscriptionStatus {
    case trial
    case active(plan: EchoelmusicWorkManager.SubscriptionPlan)
    case cancelled
    case expired
}
