...
public enum ModDestinationKey {
    ...
    // ⛔ #1302 — FOUR VOICE-STAGE DESTINATIONS STOOD HERE (#1249: `voice.harmony.mix`,
    // `voice.granular.mix`, `voice.granular.pitch`, `voice.tune.strength`) AND ARE GONE WITH
    // THE MONITOR INSERT THEY WROTE TO (founder 2026-09-12).
    //
    // ⚠️ A PERSISTED ROUTE MAY STILL NAME ONE, and that is handled rather than hoped: a
    // `ModRoute` stores its destination as a plain `String`, so it decodes fine; the engine
    // simply has no registration for it and applies nothing; and `displayName` below returns
    // an unknown key AS-IS, so the matrix row renders the raw string instead of vanishing.
    // That is the designed behaviour for "an older or newer build's key" and it covers this
    // removal without a migration.
