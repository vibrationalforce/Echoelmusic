...
class SocialHealthSupport: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.echoelmusic", category: "SocialHealthSupport")
    ...
    func assessSocialHealth() {
        ...
        logger.debug("📊 Social Health Assessment:")
        logger.debug("   - Social Connection Score: \(String(format: "%.1f", socialConnectionScore))/100")
        logger.debug("   - Social Support: \(socialSupport.rawValue) (-\(String(format: "%.0f", socialSupport.mortalityRiskReduction))% mortality risk)")
        logger.debug("   - Loneliness: \(lonelinessSeverity.rawValue)")
    }
