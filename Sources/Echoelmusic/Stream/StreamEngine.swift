...
class StreamEngine: ObservableObject {
    ...
    init?(device: MTLDevice, sceneManager: SceneManager, chatAggregator: ChatAggregator, analytics: StreamAnalytics) {
        ...
        guard let queue = device.makeCommandQueue() else {
            Logger.visual("StreamEngine: Failed to create command queue", level: .error)
            return nil
        }
        ...
        Logger.visual("StreamEngine: Initialized", level: .info)
    }
    ...
    func startStreaming(destinations: [StreamDestination], streamKeys: [StreamDestination: String]) async throws {
        ...
        for destination in destinations {
            ...
            Logger.visual("StreamEngine: Connected to \(destination.rawValue)")
        }
        ...
        Logger.visual("StreamEngine: Started streaming to \(destinations.count) destination(s)", level: .info)
    }
    ...
    func stopStreaming() {
        ...
        for (destination, client) in rtmpClients {
            client.disconnect()
            Logger.visual("StreamEngine: Disconnected from \(destination.rawValue)")
        }
        ...
        Logger.visual("StreamEngine: Stopped streaming")
    }
        ...
        for (destination, client) in rtmpClients {
            Task {
                do {
                    ...
                    Logger.visual("StreamEngine: Failed to send frame to \(destination.rawValue) - \(error)", level: .error)
    ...
    func switchScene(to scene: Scene, transition: SceneTransition = .cut) async {
        ...
        Logger.visual("StreamEngine: Switched to scene '\(scene.name)' with \(transition.rawValue) transition")
    }
    ...
    func configureBioReactiveSceneSwitching(enabled: Bool, rules: [BioSceneRule]) {
        ...
        Logger.visual("StreamEngine: Bio-reactive scene switching \(enabled ? "enabled" : "disabled") with \(rules.count) rules")
    }
    ...
    func enableAdaptiveBitrate(_ enabled: Bool) {
        ...
        Logger.visual("StreamEngine: Adaptive bitrate \(enabled ? "enabled" : "disabled")")
    }
    ...
    func updateNetworkConditions(packetLoss: Double, rtt: TimeInterval) {
        ...
        if encodingManager.adaptiveBitrateEnabled {
            if packetLoss > 0.02 {
                ...
                Logger.visual("StreamEngine: Reduced bitrate to \(bitrate) kbps due to packet loss", level: .warning)
                ...
                encodingManager.updateBitrate(bitrate)
                Logger.visual("StreamEngine: Increased bitrate to \(bitrate) kbps")
            }
        }
    }
}
...
class EncodingManager {
    ...
    func startEncoding(resolution: StreamEngine.Resolution, frameRate: Int, bitrate: Int) throws {
        ...
        Logger.visual("EncodingManager: Started encoding at \(resolution.rawValue) @ \(frameRate) FPS, \(bitrate) kbps", level: .info)
    }
