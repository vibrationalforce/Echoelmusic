...
class StreamEngine: ObservableObject {
    ...
    init?(device: MTLDevice, sceneManager: SceneManager, chatAggregator: ChatAggregator, analytics: StreamAnalytics) {
        ...
        guard let queue = device.makeCommandQueue() else {
            logger.error("StreamEngine: Failed to create command queue")
            return nil
        }
    ...
    func startStreaming(destinations: [StreamDestination], streamKeys: [StreamDestination: String]) async throws {
        ...
        for destination in destinations {
            ...
            logger.info("StreamEngine: Connected to \(destination.rawValue, privacy: .public)")
        }
        ...
        logger.info("StreamEngine: Started streaming to \(destinations.count, privacy: .public) destination(s)")
    }
    ...
    func stopStreaming() {
        ...
        for (destination, client) in rtmpClients {
            client.disconnect()
            logger.info("StreamEngine: Disconnected from \(destination.rawValue, privacy: .public)")
        }
        ...
        logger.info("StreamEngine: Stopped streaming")
    }
        ...
        for (destination, client) in rtmpClients {
            Task {
                do {
                    ...
                    self.logger.error("StreamEngine: Failed to send frame to \(destination.rawValue, privacy: .public) - \(error.localizedDescription, privacy: .public)")
    ...
    func switchScene(to scene: Scene, transition: SceneTransition = .cut) async {
        ...
        logger.info("StreamEngine: Switched to scene '\(scene.name, privacy: .public)' with \(transition.rawValue, privacy: .public) transition")
    }
    ...
    func configureBioReactiveSceneSwitching(enabled: Bool, rules: [BioSceneRule]) {
        ...
        logger.info("StreamEngine: Bio-reactive scene switching \(enabled ? "enabled" : "disabled", privacy: .public) with \(rules.count, privacy: .public) rules")
    }
    ...
    func enableAdaptiveBitrate(_ enabled: Bool) {
        ...
        logger.info("StreamEngine: Adaptive bitrate \(enabled ? "enabled" : "disabled", privacy: .public)")
    }
    ...
    func updateNetworkConditions(packetLoss: Double, rtt: TimeInterval) {
        ...
        if encodingManager.adaptiveBitrateEnabled {
            if packetLoss > 0.02 {
                ...
                logger.warning("StreamEngine: Reduced bitrate to \(bitrate, privacy: .public) kbps due to packet loss")
                ...
                encodingManager.updateBitrate(bitrate)
                logger.info("StreamEngine: Increased bitrate to \(bitrate, privacy: .public) kbps")
            }
        }
    }
}
...
class EncodingManager {
    ...
    private let encodingLogger = Logger(subsystem: "com.echoelmusic", category: "Encoding")
    ...
    func startEncoding(resolution: StreamEngine.Resolution, frameRate: Int, bitrate: Int) throws {
        ...
        encodingLogger.info("EncodingManager: Started encoding at \(resolution.rawValue, privacy: .public) @ \(frameRate, privacy: .public) FPS, \(bitrate, privacy: .public) kbps")
    }
