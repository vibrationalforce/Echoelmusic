...
class QualityAssuranceSystem: ObservableObject {
    ...
    init() {
        ...
        logger.info("Ready for comprehensive quality testing")
    }
    ...
    func runCompleteTestSuite() async {
        ...
        logger.info("Overall Quality Score: \(String(format: "%.1f", self.overallQualityScore), privacy: .public)%")
        logger.info("Issues Found: \(self.issues.count, privacy: .public)")
    }
    ...
    private func runTest(name: String, category: QualityTest.TestCategory, duration: Double) async {
        ...
        logger.debug("Running: \(name, privacy: .public)...")
        ...
        logger.debug("\(name, privacy: .public): \(String(format: "%.1f", score), privacy: .public)% [\(test.status.rawValue, privacy: .public)]")
    }
