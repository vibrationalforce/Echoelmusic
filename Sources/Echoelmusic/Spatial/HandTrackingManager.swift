...
class HandTrackingManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.echoelmusic", category: "HandTracking")
    ...
    func processFrame(_ pixelBuffer: CVPixelBuffer) {
        ...
        do {
            ...
            logger.error("Hand tracking error: \(error.localizedDescription)")
        }
    }
    ...
    private func handleHandPoseRequest(request: VNRequest, error: Error?) {
        if let error = error {
            logger.error("Hand pose request error: \(error.localizedDescription)")
            return
        }
