...
public class ARFaceTrackingManager: NSObject, ObservableObject {
    ...
    public func start() {
        guard ARFaceTrackingConfiguration.isSupported else {
            logger.error("Face tracking not supported on this device")
            return
        }
    ...
    /// Stop face tracking
    public func stop() {
        logger.info("Stopping face tracking...")
        arSession?.pause()
        isTracking = false
        blendShapes = [:]
        headTransform = nil
    ...
    public func reset() {
        logger.info("Resetting face tracking...")
        stop()
        start()
    }
...
extension ARFaceTrackingManager: ARSessionDelegate {
    ...
    public func sessionInterruptionEnded(_ session: ARSession) {
        logger.info("Session interruption ended, restarting...")
        reset()
    }
}
