...
class ChromaKeyEngine: ObservableObject {
    ...
    init?() {
        ...
        guard let device = MTLCreateSystemDefaultDevice() else {
            // Cannot use logger before full initialization
            return nil
        }
        self.device = device
        ...
        do {
            try compilePipeline()
            logger.info("✅ ChromaKeyEngine: Initialized successfully")
        } catch {
            logger.error("❌ ChromaKeyEngine: Failed to compile shaders - \(error.localizedDescription)")
            return nil
        }
    }
    ...
    private func compilePipeline() throws {
        for pass in ShaderPass.allCases {
            ...
            logger.debug("✅ ChromaKeyEngine: Compiled shader pass '\(pass.rawValue)'")
        }
    }
    ...
    func start() {
        ...
        isActive = true
        logger.info("▶️ ChromaKeyEngine: Started")
    }
    ...
    func stop() {
        ...
        logger.info("⏹️ ChromaKeyEngine: Stopped")
    }
    ...
    func autoCalibrate(from texture: MTLTexture) async throws {
        ...
        logger.info("✅ ChromaKeyEngine: Auto-calibration completed in \(String(format: "%.1f", elapsedTime))ms - Tolerance: \(String(format: "%.3f", self.tolerance)), Sampled \(self.calibrationPoints.count) points")
    }
    ...
    func process(sourceTexture: MTLTexture, backgroundTexture: MTLTexture? = nil) async throws -> MTLTexture {
        ...
        logger.debug("🎬 ChromaKeyEngine: Processed frame in \(String(format: "%.1f", elapsedTime))ms (\(String(format: "%.0f", self.currentFPS)) FPS)")
    ...
    func applyPreset(_ preset: ChromaKeyPreset) {
        ...
        logger.info("🎨 ChromaKeyEngine: Applied preset '\(preset.name)'")
    }
