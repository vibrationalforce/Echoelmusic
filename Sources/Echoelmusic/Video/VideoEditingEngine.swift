...
class VideoEditingEngine: ObservableObject {
    ...
    init(timeline: Timeline = Timeline()) {
        ...
        Logger.log("VideoEditingEngine: Initialized with Undo/Redo support", category: .system, level: .info)
    }
    ...
    func addClip(_ clip: VideoClip, to track: Track, at time: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Added clip '\(clip.name)' to track '\(track.name)' at \(snappedTime.seconds)s")
    }
    ...
    func removeClip(_ clipID: UUID, from track: Track) {
        ...
        undoManager.execute(command)
        Logger.visual("VideoEditingEngine: Removed clip '\(clip.name)' from track '\(track.name)'")
    }
    ...
    func moveClip(_ clipID: UUID, from sourceTrack: Track, to destinationTrack: Track, at time: CMTime) {
        ...
        undoManager.execute(command)
        Logger.visual("VideoEditingEngine: Moved clip '\(clip.name)' to track '\(destinationTrack.name)'")
    }
    ...
    func rippleEdit(clipID: UUID, track: Track, newDuration: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Ripple edit - shifted \(track.clips.count - index - 1) clips by \(delta.seconds)s")
    }
    ...
    func rollEdit(leftClipID: UUID, rightClipID: UUID, track: Track, newCutPoint: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Roll edit - moved cut point to \(newCutPoint.seconds)s")
    }
    ...
    func slipEdit(clipID: UUID, track: Track, newInPoint: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Slip edit - new in point: \(newInPoint.seconds)s")
    }
    ...
    func slideEdit(clipID: UUID, track: Track, newStartTime: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Slide edit - moved clip to \(newStartTime.seconds)s")
    }
    ...
    func splitClip(clipID: UUID, track: Track, at time: CMTime) -> (UUID, UUID)? {
        ...
        undoManager.execute(command)
        Logger.visual("VideoEditingEngine: Split clip '\(originalClip.name)' at \(time.seconds)s")
    ...
    func addKeyframe(clipID: UUID, track: Track, property: KeyframeProperty, at time: CMTime, value: Float) {
        ...
        Logger.visual("VideoEditingEngine: Added keyframe for \(property.rawValue) at \(time.seconds)s")
    }
    ...
    func removeKeyframe(clipID: UUID, track: Track, property: KeyframeProperty, at time: CMTime) {
        ...
        Logger.visual("VideoEditingEngine: Removed keyframe for \(property.rawValue)")
    }
    ...
    func play() {
        ...
        do {
            ...
            Logger.visual("VideoEditingEngine: Started playback", level: .info)
        } catch {
            Logger.visual("VideoEditingEngine: Failed to build composition - \(error)", level: .error)
        }
    }
    ...
    func pause() {
        ...
        Logger.visual("VideoEditingEngine: Paused playback")
    }
    ...
    func stopPlayback() {
        ...
        Logger.visual("VideoEditingEngine: Stopped playback")
    }
    ...
    func addMarker(at time: CMTime, label: String, color: MarkerColor) {
        ...
        Logger.visual("VideoEditingEngine: Added marker '\(label)' at \(time.seconds)s")
    }
...
extension VideoEditingEngine {
    ...
    func addTextOverlay(_ overlay: TextOverlay) {
        ...
        Logger.visual("VideoEditingEngine: Added text overlay '\(overlay.text)' at \(overlay.startTime.seconds)s")
    }
