...
class DeviceCapabilities: ObservableObject {
    ...
    private func detectDeviceModel() {
        ...
        logger.info("📱 Device: \(self.deviceModel) (\(identifier))")
    }
    ...
    private func detectASAFSupport() {
        ...
        if supportsASAF {
            logger.info("✅ ASAF Supported (iOS \(majorVersion)+ with \(self.deviceModel))")
        } else {
            logger.warning("⚠️ ASAF Not Supported - Need iOS 19+ and iPhone 16+ (Current: iOS \(self.iOSVersion), \(self.deviceModel))")
        }
    }
    ...
    private func detectAirPods() {
        ...
        for output in outputs {
            ...
            if portType == .bluetoothA2DP || portType == .bluetoothHFP || portType == .bluetoothLE {
                ...
                logger.info("🎧 Audio Output: \(self.airPodsModel ?? "Unknown")\(self.supportsAPACCodec ? " - APAC Codec Available" : "")")
    ...
    @objc private func audioRouteChanged(notification: Notification) {
        logger.debug("🔄 Audio route changed, re-detecting AirPods...")
        detectAirPods()
    }
