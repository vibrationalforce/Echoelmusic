...
private let pianoRollLogger = Logger(subsystem: "com.echoelmusic", category: "PianoRollView")
...
class PianoRollViewModel: ObservableObject {
    ...
    func connect(midi2: MIDI2Manager, mpe: MPEZoneManager) {
        ...
        pianoRollLogger.info("🎹 PianoRoll: Connected to MIDI 2.0 + MPE")
    }
    ...
    func addNote(at location: CGPoint, scale: CGFloat) {
        ...
        notes.append(note)
        pianoRollLogger.debug("🎹 Added note: \(note.pitch) at beat \(note.startBeat)")
    }
    ...
    private func triggerNoteOn(_ note: MIDINote) {
        if useMPE, let mpe = mpeZoneManager {
            ...
            if let voice = mpe.allocateVoice(note: UInt8(note.pitch), velocity: note.velocity32bit) {
                ...
                pianoRollLogger.debug("🎹 MPE Note On: \(note.pitch) vel=\(note.velocity) ch=\(voice.channel + 1)")
            }
            ...
            pianoRollLogger.debug("🎹 MIDI2 Note On: \(note.pitch) vel=\(note.velocity)")
        }
    }
    ...
    private func triggerNoteOff(_ note: MIDINote) {
        if useMPE, let mpe = mpeZoneManager {
            if let voice = activeVoices[note.id] {
                ...
                pianoRollLogger.debug("🎹 MPE Note Off: \(note.pitch)")
            }
        } else if let midi2 = midi2Manager {
            midi2.sendNoteOff(channel: 0, note: UInt8(note.pitch))
            pianoRollLogger.debug("🎹 MIDI2 Note Off: \(note.pitch)")
        }
    }
    ...
    func exportToMIDI() -> Data? {
        ...
        pianoRollLogger.info("📤 Exporting \(self.notes.count) notes to MIDI")
        return nil
    }
