...
class MPEZoneManager: ObservableObject {
    ...
    private let midiLogger = Logger(subsystem: "com.echoelmusic", category: "MPEZoneManager")
    ...
    func allocateVoice(note: UInt8, velocity: Float) -> MPEVoice? {
        ...
        if voices.count >= maxVoices {
            ...
            guard let stolenVoice = selectVoiceToSteal() else {
                midiLogger.warning("Failed to steal voice")
                return nil
            }
        ...
        guard let channel = findAvailableChannel() else {
            midiLogger.warning("No available channel")
            return nil
        }
        ...
        midiLogger.debug("Allocated voice: Note \(note) on channel \(channel + 1)")
    ...
    func deallocateVoice(voice: MPEVoice) {
        ...
        midiLogger.debug("Deallocated voice: Note \(voice.note) from channel \(voice.channel + 1)")
    }
    ...
    func releaseAllVoices() {
        ...
        midiLogger.info("Released all voices")
    }
    ...
    func sendMPEConfiguration(memberChannels: UInt8 = 15) {
        ...
        midiLogger.info("Sent configuration: \(memberChannels) member channels")
    }
    ...
    func setPitchBendRange(semitones: UInt8 = 48) {
        ...
        midiLogger.info("Set pitch bend range: ±\(semitones) semitones")
    }
}
