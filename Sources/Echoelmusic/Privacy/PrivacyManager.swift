...
class PrivacyManager: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.echoelmusic", category: "PrivacyManager")
    ...
    init() {
        ...
        logger.info("✅ Privacy Manager: Initialized - Mode: \(self.privacyMode.rawValue), Local-first, Zero trackers")
    }
    ...
    func setPrivacyMode(_ mode: PrivacyMode) {
        ...
        if !mode.allowsCloudSync {
            cloudSyncEnabled = false
            logger.info("🔒 Cloud sync disabled (privacy mode: \(mode.rawValue))")
        }

        if !mode.allowsAnalytics {
            analyticsEnabled = false
            logger.info("🔒 Analytics disabled (privacy mode: \(mode.rawValue))")
        }
    ...
    private func loadKeyFromKeychain() -> Data? {
        ...
        guard status == errSecSuccess else {
            logger.debug("🔐 Keychain load failed or key not found: \(status)")
            return nil
        }
    ...
    private func saveKeyToKeychain(_ key: SymmetricKey) {
        ...
        if status == errSecSuccess {
            ...
            logger.warning("⚠️ Keychain save failed: \(status)")
        }
    }
    ...
    func exportAllUserData() async throws -> URL {
        ...
        logger.info("✅ User data exported to: \(tempURL.path)")
        return tempURL
    }
    ...
    func calculateStorageUsage() async {
        ...
        logger.info("💾 Storage Usage: Local: \(ByteCountFormatter.string(fromByteCount: self.localStorageUsed, countStyle: .file))\(self.cloudSyncEnabled ? ", Cloud: \(ByteCountFormatter.string(fromByteCount: self.cloudStorageUsed, countStyle: .file))" : "")")
    }
