...
class AdaptiveQualityManager {
    ...
    var currentQuality: QualityLevel = .high {
        didSet {
            if currentQuality != oldValue {
                ...
                logger.info("📊 Quality Level changed: \(oldValue.rawValue) → \(self.currentQuality.rawValue)")
            }
        }
    }
    ...
    private func transitionToQuality(_ newQuality: QualityLevel) {
        logger.info("🔄 Transitioning quality: \(self.currentQuality.rawValue) → \(newQuality.rawValue) - FPS: \(String(format: "%.1f", self.metrics.averageFPS)) | CPU: \(String(format: "%.1f%%", self.metrics.cpuUsage * 100)) | GPU: \(String(format: "%.1f%%", self.metrics.gpuUsage * 100))")
    ...
    private func handleMemoryWarning() {
        logger.warning("⚠️ Memory Warning! Degrading quality immediately.")
