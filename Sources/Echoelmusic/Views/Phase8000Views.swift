...
// MARK: - Wellness Session View

/// Wellness and meditation session interface
/// DISCLAIMER: For general wellness only, NOT medical advice
public struct WellnessSessionView: View {
    @StateObject private var engine = WellnessTrackingEngine()
    @State private var selectedCategory: WellnessCategory = .meditation
    @State private var selectedPattern: BreathingPattern?
    @State private var showingDisclaimer = true
    @State private var moodBefore: WellnessSession.MoodLevel = .neutral
    ...
    private var activeSessionCard: some View {
        VStack(spacing: 16) {
            if let session = engine.currentSession {
                ...
                CoherenceRing(coherence: engine.currentCoherence)
                ...
                VStack {
                    Text("How do you feel?")
                        .font(.headline)
                    HStack(spacing: 20) {
                        ForEach(WellnessSession.MoodLevel.allCases, id: \.self) { mood in
                            Button {
                                engine.endSession(moodAfter: mood)
                            } label: {
                                Text(mood.emoji)
                                    .font(.title)
                            }
                ...
                Button("End Session") {
                    engine.endSession(moodAfter: .neutral)
                }
                .foregroundColor(.red)
            }
        }
    ...
    private var categorySelector: some View {
        VStack(alignment: .leading) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack {
                    ForEach([WellnessCategory.meditation, .breathwork, .relaxation, .focus, .sleepSupport], id: \.self) { cat in
                        CategoryPill(category: cat, isSelected: selectedCategory == cat) {
                            selectedCategory = cat
                            engine.selectedCategory = cat
                        }
                    }
                }
            }
    ...
    private var quickStartSection: some View {
        VStack(alignment: .leading) {
            Text("Quick Start")
                .font(.headline)
            Button {
                engine.startSession(name: "Quick \(selectedCategory.rawValue)", category: selectedCategory, moodBefore: moodBefore)
            } label: {
                HStack {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                ForEach(BreathingPattern.allPatterns, id: \.id) { pattern in
                    BreathingPatternCard(pattern: pattern) {
                        engine.startBreathingExercise(pattern)
                    }
                }
            }
        }
    ...
    private var statsSection: some View {
        VStack(alignment: .leading) {
            Text("Your Progress")
                .font(.headline)
            HStack {
                StatCard(title: "Sessions", value: "\(engine.statistics.totalSessions)", icon: "figure.mind.and.body")
                StatCard(title: "Minutes", value: "\(engine.statistics.totalMinutes)", icon: "clock")
                StatCard(title: "Streak", value: "\(engine.statistics.currentStreak)", icon: "flame")
            }
        }
    }
