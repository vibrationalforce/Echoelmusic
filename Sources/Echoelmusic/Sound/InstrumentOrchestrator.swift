...
class InstrumentOrchestrator: ObservableObject {
    ...
    private init() {
        ...
        Logger.audio("InstrumentOrchestrator: Initialized, Instruments=\(soundLibrary.availableInstruments.count), Synth Engines=\(soundLibrary.availableSynthEngines.count)", level: .info)
    }
    ...
    private func setupAudioEngine() {
        ...
        guard let engine = audioEngine else {
            Logger.audio("InstrumentOrchestrator: Failed to create AVAudioEngine", level: .error)
            return
        }
        ...
        guard let format = AVAudioFormat(standardFormatWithSampleRate: 48000, channels: 2) else {
            Logger.audio("InstrumentOrchestrator: Failed to create audio format", level: .error)
            return
        }
        ...
        do {
            try engine.start()
            isPlaying = true
            Logger.audio("InstrumentOrchestrator: Audio engine started", level: .info)
        } catch {
            Logger.audio("InstrumentOrchestrator: Failed to start audio engine: \(error)", level: .error)
        }
    }
    ...
    func selectInstrument(name: String) {
        if let instrument = soundLibrary.availableInstruments.first(where: { $0.name == name }) {
            ...
            Logger.audio("Selected: \(instrument.name)")
        }
    }
    ...
    func selectInstrument(index: Int) {
        ...
        Logger.audio("Selected: \(currentInstrument?.name ?? "Unknown")")
    }
    ...
    func selectSynthEngine(_ type: UniversalSoundLibrary.SynthEngine.SynthType) {
        if let engine = soundLibrary.availableSynthEngines.first(where: { $0.type == type }) {
            ...
            Logger.audio("Synth Engine: \(engine.name)")
        }
    }
    ...
    func noteOn(midiNote: Int, velocity: Float = 0.8) {
        guard let synthEngine = currentSynthEngine else {
            Logger.audio("No synthesis engine selected", level: .warning)
            return
        }
        ...
        if let buffer = createAudioBuffer(from: envelopedSamples, sampleRate: Double(sampleRate)) {
            ...
            Logger.midi("Note On: MIDI \(midiNote) @ \(Int(velocity * 100))% velocity")
        }
    }
    ...
    func noteOff(midiNote: Int) {
        ...
        if let index = voices.firstIndex(where: { $0.midiNote == midiNote && $0.isActive }) {
            ...
            Logger.midi("Note Off: MIDI \(midiNote)")
        }
    }
    ...
    func triggerDrum(_ drumType: DrumType, velocity: Float = 0.8) {
        ...
        guard let physicalEngine = soundLibrary.availableSynthEngines.first(where: { $0.type == .physicalModeling }) else {
            Logger.audio("Physical modeling engine not available", level: .warning)
            return
        }
        ...
        if let buffer = createAudioBuffer(from: samples, sampleRate: Double(sampleRate)) {
            ...
            Logger.audio("Drum: \(drumType) @ \(Int(velocity * 100))%")
        }
    }
