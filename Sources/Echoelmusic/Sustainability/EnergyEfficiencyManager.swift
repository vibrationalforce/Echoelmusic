...
class EnergyEfficiencyManager: ObservableObject {
    ...
    init() {
        ...
        logger.info("✅ EnergyEfficiencyManager initialized")
        logger.info("🌱 Eco Mode: \(self.ecoModeEnabled ? "Enabled" : "Disabled")")
        logger.info("⚡️ Efficiency Level: \(self.currentEnergyEfficiency.rawValue)")
    }
    ...
    private func detectPowerSource() {
        ...
        case .charging, .full:
            ...
            if isRenewable {
                ...
                logger.info("♻️ Plugged in to renewable energy source")
            } else {
        ...
        case .unplugged:
            ...
            logger.info("🔋 Running on battery (\(Int(batteryLevel * 100))%)")
    ...
    func enableEcoMode() {
        ...
        logger.info("🌱 Eco Mode: Enabled - CPU: 30%, GPU: 40%, FPS: 30, Dark mode, No background")
    ...
    private func adjustForPowerSource() {
        ...
        case .battery(let level):
            if level < 0.2 {  // < 20%
                logger.warning("🔋 Low battery - enabling eco mode")
                enableEcoMode()
        ...
        case .pluggedIn(let isRenewable):
            if isRenewable {
                ...
                logger.info("♻️ Renewable energy detected - performance mode enabled")
            } else {
    ...
    func endSession() -> EnergyMetrics {
        ...
        logger.info("🌱 Session ended - Duration: \(Int(duration))s, Energy: \(String(format: "%.2f", metrics.totalEnergyKWh)) kWh, Carbon: \(String(format: "%.2f", self.estimatedCarbonFootprint)) g CO2e")
    ...
    func setRenewableEnergy(_ isRenewable: Bool) {
        ...
        if isRenewable {
            carbonIntensity = .renewableEstimate
            logger.info("♻️ Renewable energy mode: Enabled - Carbon footprint ~12x lower!")
        } else {
