...
class MIDIController: ObservableObject {
    ...
    private func handleParameterChange(parameter: MIDIParameter, value: Float) {
        logger.debug("🎹 MIDI parameter: \(parameter) = \(value)")
    ...
    func addMapping(_ mapping: MIDIMapping) {
        ...
        logger.debug("🎹 Added MIDI mapping: CC\(mapping.controllerNumber) → \(mapping.parameter)")
    }

    /// Remove all mappings
    func clearMappings() {
        mappings.removeAll()
        logger.debug("🎹 Cleared all MIDI mappings")
    }
    ...
    func startLearnMode(for parameter: MIDIParameter, minValue: Float, maxValue: Float, timeout: TimeInterval = 10.0) async -> UInt8? {
        ...
        return await withCheckedContinuation { continuation in
            ...
            handler = { [weak self] message in
                ...
                // Found CC controller
                self?.logger.info("🎹 MIDI Learn: Learned CC\(ccNumber)")
            ...
            guard let validHandler = handler else {
                continuation.resume(returning: nil)
                return
            }
            messageHandlers.append(validHandler)

            // Timeout
            timeoutTask = Task {
                try? await Task.sleep(nanoseconds: UInt64(timeout * 1_000_000_000))

                self.logger.warning("🎹 MIDI Learn: Timeout")
