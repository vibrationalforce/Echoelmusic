...
class MIDIController: ObservableObject {
    ...
    private func setupMIDI() {
        ...
        guard status == noErr else {
            Logger.midi("Failed to create MIDI client: \(status)", level: .error)
            return
        }
        ...
        guard status == noErr else {
            Logger.midi("Failed to create MIDI input port: \(status)", level: .error)
            return
        }
        ...
        Logger.midi("MIDI controller initialized")
    }
    ...
    private func connectToAllSources() {
        ...
        for i in 0..<sourceCount {
            ...
            if status == noErr {
                ...
                Logger.midi("Connected to MIDI device: \(device.name)")
            }
        }
    ...
    private func handleParameterChange(parameter: MIDIParameter, value: Float) {
        Logger.midi("MIDI parameter: \(parameter) = \(value)")
    ...
    func addMapping(_ mapping: MIDIMapping) {
        ...
        Logger.midi("Added MIDI mapping: CC\(mapping.controllerNumber) → \(mapping.parameter)")
    }

    /// Remove all mappings
    func clearMappings() {
        mappings.removeAll()
        Logger.midi("Cleared all MIDI mappings")
    }
    ...
    func setupDefaultMappings() {
        ...
        Logger.midi("Setup default MIDI mappings")
    }
...
extension MIDIController {
    ...
    func startLearnMode(for parameter: MIDIParameter, minValue: Float, maxValue: Float, timeout: TimeInterval = 10.0) async -> UInt8? {
        Logger.midi("MIDI Learn: Waiting for controller input...")
        ...
        return await withCheckedContinuation { continuation in
            ...
            handler = { [weak self] message in
                ...
                // Found CC controller
                Logger.midi("MIDI Learn: Learned CC\(ccNumber)")
            ...
            timeoutTask = Task {
                ...
                Logger.midi("MIDI Learn: Timeout")
