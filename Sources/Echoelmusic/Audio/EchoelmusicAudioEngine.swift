...
@MainActor
final class EchoelmusicAudioEngine: ObservableObject {
    static let shared = EchoelmusicAudioEngine()
    ...
    func initialize() async throws {
        ...
        print("✅ EchoelmusicAudioEngine initialized - Latency: \(currentLatency * 1000)ms")
    }
...
struct Instrument: Identifiable {
    let id = UUID()
    let type: EchoelmusicAudioEngine.InstrumentType
}

struct AudioEffect: Identifiable {
    let id = UUID()
    let type: EchoelmusicAudioEngine.EffectType
