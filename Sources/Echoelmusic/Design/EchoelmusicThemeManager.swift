//
//  EchoelmusicThemeManager.swift
//  Echoelmusic
//
//  Created: 2025-11-25
//  Copyright © 2025 Echoelmusic. All rights reserved.
//
//  Echoelmusic Theme Manager
...
/// Central theme manager for Echoelmusic
@MainActor
class EchoelmusicThemeManager: ObservableObject {
    static let shared = EchoelmusicThemeManager()
...
struct ThemePreviewCard: View {
    let theme: EchoelmusicThemeManager.Theme
    @ObservedObject var themeManager = EchoelmusicThemeManager.shared
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: VaporwaveDesignSystem.Spacing.large) {
            ...
            LazyVGrid(
                ...
                ForEach(EchoelmusicThemeManager.Theme.allCases.filter { $0 != .custom }) { theme in
...
#Preview("Theme Manager") {
    ZStack {
        EchoelmusicThemeManager.shared.background
            .ignoresSafeArea()

        ScrollView {
            VStack(spacing: VaporwaveDesignSystem.Spacing.xlarge) {
                ChromeText(text: "Echoelmusic", font: VaporwaveDesignSystem.Typography.display)

                ThemeSelectorView()

                // Example components with current theme
                LiquidGlassCard(glowColor: EchoelmusicThemeManager.shared.primary) {
                    VStack(spacing: VaporwaveDesignSystem.Spacing.medium) {
                        HStack {
                            NeonIcon(
                                systemName: EchoelmusicIcon.Music.waveformCircle,
                                color: EchoelmusicThemeManager.shared.primary,
                                size: 48
                            )

                            NeonIcon(
                                systemName: EchoelmusicIcon.Transport.playCircle,
                                color: EchoelmusicThemeManager.shared.secondary,
                                size: 48
                            )

                            NeonIcon(
                                systemName: EchoelmusicIcon.Transport.recordFill,
                                color: EchoelmusicThemeManager.shared.accent,
                                size: 48
                            )
                        }
                        ...
                        LiquidGlassButton(
                            ...
                            action: {},
                            glowColor: EchoelmusicThemeManager.shared.primary
                        )
                    }
                }
            }
