...
class Push3LEDController: ObservableObject {
    ...
    private func setupMIDI() {
        ...
        guard clientStatus == noErr else {
            logger.warning("Failed to create MIDI client for Push 3")
            return
        }
        midiClient = client
        ...
        guard portStatus == noErr else {
            logger.warning("Failed to create MIDI output port for Push 3")
            return
        }
        outputPort = port
        ...
        for i in 0..<destinationCount {
            ...
            if let deviceName = name?.takeRetainedValue() as String? {
                if deviceName.contains("Ableton Push 3") || deviceName.contains("Push 3") {
                    ...
                    isConnected = true
                    logger.info("Found Push 3: \(deviceName, privacy: .public)")
                    return
                }
            }
        }

        logger.warning("Push 3 not found. Connect via USB and retry.")
    }
    ...
    func setGrid(_ grid: [[RGB]]) {
        guard grid.count == 8, grid.allSatisfy({ $0.count == 8 }) else {
            logger.warning("Invalid grid dimensions (must be 8x8)")
            return
        }
        ledGrid = grid
    }
    ...
    func sendGridToHardware() {
        ...
        if status != noErr {
            logger.warning("Failed to send SysEx to Push 3: \(status, privacy: .public)")
        }
    }
    ...
    func flashGesture(gesture: String) {
        ...
        logger.info("Gesture flash: \(gesture, privacy: .public)")
    }
    ...
    func applyPattern(_ pattern: LEDPattern) {
        ...
        logger.info("Push 3 pattern: \(pattern.rawValue, privacy: .public)")
    }
