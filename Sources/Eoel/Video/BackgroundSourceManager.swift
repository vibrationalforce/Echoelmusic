...
/// Supports multiple background types: images, videos, Eoel visuals, virtual backgrounds
...
class BackgroundSourceManager: ObservableObject {
    ...
    // MARK: - Eoel Visual Integration

    private var eoelVisualRenderer: EoelVisualRenderer?
    ...
    enum BackgroundSource: Identifiable, Equatable {
        ...
        case eoelVisual(EoelVisualType)
        ...
        var id: String {
            ...
            case .eoelVisual(let type): return "eoel_\(type.rawValue)"
        ...
        var displayName: String {
            ...
            case .eoelVisual(let type): return "Eoel: \(type.displayName)"
    ...
    // MARK: - Eoel Visual Types

    enum EoelVisualType: String, CaseIterable {
    ...
    private func initializeDefaultSources() {
        availableSources = [
            ...
            // Eoel Visuals (all bio-reactive)
            .eoelVisual(.cymatics),
            .eoelVisual(.mandala),
            .eoelVisual(.particles),
            .eoelVisual(.waveform),
            .eoelVisual(.spectral),
    ...
    func setSource(_ source: BackgroundSource) async throws {
        ...
        do {
            ...
            case .eoelVisual(let type):
                try await startEoelVisual(type: type)
    ...
    func getCurrentTexture(size: CGSize) async throws -> MTLTexture {
        ...
        case .video, .liveCamera, .eoelVisual, .gradient(.bioreactive, _):
    ...
    private func startEoelVisual(type: EoelVisualType) async throws {
        // TODO: Integrate with existing Eoel visual renderers
        // (CymaticsRenderer, MandalaRenderer, ParticleSystem, etc.)

        // For now, create placeholder
        eoelVisualRenderer = EoelVisualRenderer(device: device, type: type)

        print("🎨 BackgroundSourceManager: Started Eoel visual '\(type.displayName)'")
    }
    ...
    private func updateAnimatedSource(size: CGSize) async throws -> MTLTexture {
        ...
        case .eoelVisual(let type):
            return try await updateEoelVisual(type: type, size: size)
    ...
    private func updateEoelVisual(type: EoelVisualType, size: CGSize) async throws -> MTLTexture {
        guard let renderer = eoelVisualRenderer else {
            throw BackgroundError.eoelVisualNotActive
        }
...
// MARK: - Eoel Visual Renderer (Placeholder)

@MainActor
class EoelVisualRenderer {
    private let device: MTLDevice
    private let type: BackgroundSourceManager.EoelVisualType

    private var hrvCoherence: Float = 0.5
    private var heartRate: Float = 70.0

    init(device: MTLDevice, type: BackgroundSourceManager.EoelVisualType) {
        self.device = device
        self.type = type
    }
    ...
    func render(size: CGSize) async throws -> MTLTexture {
        // TODO: Integrate with actual Eoel visual renderers
...
enum BackgroundError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .eoelVisualNotActive:
            return "Eoel visual renderer is not active"
        case .textureCreationFailed:
