...
class NodeGraph: ObservableObject {
    ...
    private let audioQueue = DispatchQueue(
        label: "com.eoel.nodegraph.audio",
        qos: .userInteractive
    )
    ...
    /// Get node by ID
    func node(withID id: UUID) -> EoelNode? {
    ...
    func savePreset(name: String) -> NodeGraphPreset {
        let nodeManifests = nodes.map { node in
            (node as? BaseEoelNode)?.createManifest()
        }.compactMap { $0 }
