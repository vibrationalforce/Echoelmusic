...
final class CircadianWellnessTests: XCTestCase {
    ...
    // MARK: - Lifestyle Coach Engine Tests

    @MainActor
    func testLifestyleCoachInitialization() async {
        let coach = LifestyleCoachEngine()
        XCTAssertNotNil(coach.userProfile)
        XCTAssertNotNil(coach.currentFitnessPlan)
        XCTAssertNotNil(coach.currentNutritionPlan)
    }

    @MainActor
    func testTDEECalculation() async {
        let coach = LifestyleCoachEngine()
        coach.userProfile = LifestyleCoachEngine.UserProfile(
            age: 30,
            weight: 70,
            height: 175,
            biologicalSex: .male,
        ...
        let tdee = coach.calculateTDEE()
        // TDEE should be reasonable (1500-4000 for most people)
        XCTAssertGreaterThan(tdee, 1500)
        XCTAssertLessThan(tdee, 4000)
    }

    @MainActor
    func testMacroCalculation() async {
        let coach = LifestyleCoachEngine()
        coach.userProfile.weight = 70
        coach.userProfile.fitnessGoal = .muscleGain

        let (protein, carbs, fat) = coach.calculateMacros()

        // Protein should be at least 1.6g/kg for muscle gain
        XCTAssertGreaterThanOrEqual(protein, Int(70 * 1.6))

        // Carbs and fat should be positive
        XCTAssertGreaterThan(carbs, 0)
        XCTAssertGreaterThan(fat, 0)
    }

    @MainActor
    func testFitnessPlanGeneration() async {
        let coach = LifestyleCoachEngine()
        let plan = coach.generateFitnessPlan()

        XCTAssertFalse(plan.name.isEmpty)
        XCTAssertNotNil(plan.weeklySchedule[.monday])
        XCTAssertNotNil(plan.weeklySchedule[.sunday])
    }

    @MainActor
    func testNutritionPlanGeneration() async {
        let coach = LifestyleCoachEngine()
        let plan = coach.generateNutritionPlan()

        XCTAssertGreaterThan(plan.targetCalories, 0)
        XCTAssertGreaterThan(plan.targetProtein, 0)
        XCTAssertGreaterThan(plan.meals.count, 0)
        XCTAssertGreaterThan(plan.supplements.count, 0)
    }

    @MainActor
    func testDefaultGoalsCreation() async {
        let coach = LifestyleCoachEngine()
        coach.createDefaultGoals()

        XCTAssertGreaterThan(coach.personalGoals.count, 0)

        // Check for HRV goal
        let hrvGoal = coach.personalGoals.first { $0.category == .hrv }
        XCTAssertNotNil(hrvGoal)

        // Check for coherence goal
        let coherenceGoal = coach.personalGoals.first { $0.category == .coherence }
        XCTAssertNotNil(coherenceGoal)
    }

    @MainActor
    func testProgressTracking() async {
        let coach = LifestyleCoachEngine()
        coach.updateProgress(
            waterIntake: 2.5,
            calories: 2000,
            protein: 150,
            exerciseMinutes: 45,
        ...
        XCTAssertEqual(coach.dailyProgress.waterIntake, 2.5)
        XCTAssertEqual(coach.dailyProgress.caloriesConsumed, 2000)
        XCTAssertEqual(coach.dailyProgress.proteinConsumed, 150)
        XCTAssertEqual(coach.dailyProgress.exerciseMinutes, 45)
        XCTAssertEqual(coach.dailyProgress.meditationMinutes, 20)
        XCTAssertEqual(coach.dailyProgress.sleepHours, 7.5)
        XCTAssertEqual(coach.dailyProgress.avgHRV, 55)
        XCTAssertEqual(coach.dailyProgress.avgCoherence, 0.75)
        XCTAssertEqual(coach.dailyProgress.stepsCount, 10000)
        XCTAssertEqual(coach.dailyProgress.sunlightMinutes, 30)
    }

    // MARK: - Lifestyle Health Disclaimer Tests

    func testLifestyleDisclaimersExist() {
        XCTAssertFalse(LifestyleHealthDisclaimer.fitnessDisclaimer.isEmpty)
        XCTAssertFalse(LifestyleHealthDisclaimer.nutritionDisclaimer.isEmpty)
    }

    func testLifestyleDisclaimerContent() {
        XCTAssertTrue(LifestyleHealthDisclaimer.fitnessDisclaimer.contains("Arzt"))
        XCTAssertTrue(LifestyleHealthDisclaimer.nutritionDisclaimer.contains("Ernährungsberater") ||
                      LifestyleHealthDisclaimer.nutritionDisclaimer.contains("Arzt"))
    }

    // MARK: - Integration Tests

    @MainActor
    func testCircadianPhaseAffectsRecommendations() async {
        let engine = CircadianRhythmEngine()
        engine.updateCurrentPhase()

        // Should have recommendations for current phase
        XCTAssertGreaterThan(engine.currentRecommendations.count, 0)

        // All recommendations should be relevant to current phase
        for recommendation in engine.currentRecommendations {
            XCTAssertTrue(recommendation.circadianPhases.contains(engine.currentPhase))
        }
    }

    @MainActor
    func testChronotypeAffectsFitnessPlan() async {
        let coach = LifestyleCoachEngine()

        // Test lion chronotype
        coach.userProfile.chronotype = .lion
        let lionPlan = coach.generateFitnessPlan()

        // Test wolf chronotype
        coach.userProfile.chronotype = .wolf
        let wolfPlan = coach.generateFitnessPlan()

        // Plans should be different based on chronotype
        XCTAssertNotNil(lionPlan)
        XCTAssertNotNil(wolfPlan)
    }

    @MainActor
    func testDietaryRestrictionsAffectNutritionPlan() async {
        let coach = LifestyleCoachEngine()

        // Test vegan
        coach.userProfile.dietaryRestrictions = [.vegan]
        let veganPlan = coach.generateNutritionPlan()

        // Should not contain animal products
        for meal in veganPlan.meals {
            for food in meal.foods {
                XCTAssertFalse(food.name.contains("Lachs"))
                XCTAssertFalse(food.name.contains("Eier"))
                XCTAssertFalse(food.name.contains("Hühnerbrust"))
            }
        }
    }
