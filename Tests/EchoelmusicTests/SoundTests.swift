...
final class SynthesisEngineTypeTests: XCTestCase {
    ...
    func testAllCasesCount() {
        XCTAssertEqual(SynthesisEngineType.allCases.count, 21)
    }

    // MARK: - Raw Values

    func testRawValues() {
        XCTAssertEqual(SynthesisEngineType.subtractive.rawValue, "subtractive")
        XCTAssertEqual(SynthesisEngineType.fm.rawValue, "fm")
        XCTAssertEqual(SynthesisEngineType.wavetable.rawValue, "wavetable")
        XCTAssertEqual(SynthesisEngineType.additive.rawValue, "additive")
        XCTAssertEqual(SynthesisEngineType.granular.rawValue, "granular")
        XCTAssertEqual(SynthesisEngineType.spectral.rawValue, "spectral")
        XCTAssertEqual(SynthesisEngineType.physicalModeling.rawValue, "physical_modeling")
        XCTAssertEqual(SynthesisEngineType.karplusStrong.rawValue, "karplus_strong")
        XCTAssertEqual(SynthesisEngineType.waveguide.rawValue, "waveguide")
        XCTAssertEqual(SynthesisEngineType.modal.rawValue, "modal")
        XCTAssertEqual(SynthesisEngineType.sampler.rawValue, "sampler")
        XCTAssertEqual(SynthesisEngineType.vector.rawValue, "vector")
        XCTAssertEqual(SynthesisEngineType.neural.rawValue, "neural")
        XCTAssertEqual(SynthesisEngineType.genetic.rawValue, "genetic")
        XCTAssertEqual(SynthesisEngineType.organic.rawValue, "organic")
        XCTAssertEqual(SynthesisEngineType.bioReactive.rawValue, "bio_reactive")
        XCTAssertEqual(SynthesisEngineType.echoelQuant.rawValue, "echoel_quant")
        XCTAssertEqual(SynthesisEngineType.ddsp.rawValue, "ddsp")
        XCTAssertEqual(SynthesisEngineType.cellularAutomata.rawValue, "cellular_automata")
        XCTAssertEqual(SynthesisEngineType.modalBank.rawValue, "modal_bank")
        XCTAssertEqual(SynthesisEngineType.echoelSampler.rawValue, "echoel_sampler")
    }

    func testInitFromRawValue() {
        XCTAssertEqual(SynthesisEngineType(rawValue: "fm"), .fm)
        XCTAssertEqual(SynthesisEngineType(rawValue: "ddsp"), .ddsp)
        XCTAssertEqual(SynthesisEngineType(rawValue: "bio_reactive"), .bioReactive)
        XCTAssertNil(SynthesisEngineType(rawValue: "nonexistent"))
        XCTAssertNil(SynthesisEngineType(rawValue: ""))
    }

    // MARK: - Display Names

    func testDisplayNames() {
        XCTAssertEqual(SynthesisEngineType.subtractive.displayName, "Subtractive Synthesis")
        XCTAssertEqual(SynthesisEngineType.fm.displayName, "FM Synthesis")
        XCTAssertEqual(SynthesisEngineType.bioReactive.displayName, "Bio-Reactive Synthesis")
        XCTAssertEqual(SynthesisEngineType.echoelQuant.displayName, "EchoelQuant")
        XCTAssertEqual(SynthesisEngineType.ddsp.displayName, "EchoelDDSP")
        XCTAssertEqual(SynthesisEngineType.breakbeat.displayName, "Breakbeat Chopper")
    }

    func testEveryEngineHasNonEmptyDisplayName() {
        for engine in SynthesisEngineType.allCases {
            XCTAssertFalse(engine.displayName.isEmpty, "\(engine) has empty displayName")
        }
    }

    // MARK: - Description

    func testEveryEngineHasNonEmptyDescription() {
        for engine in SynthesisEngineType.allCases {
            XCTAssertFalse(engine.description.isEmpty, "\(engine) has empty description")
        }
    }

    func testDescriptionContainsRelevantKeywords() {
        XCTAssertTrue(SynthesisEngineType.fm.description.lowercased().contains("frequency modulation"))
        XCTAssertTrue(SynthesisEngineType.karplusStrong.description.lowercased().contains("plucked string"))
        XCTAssertTrue(SynthesisEngineType.bioReactive.description.lowercased().contains("heart rate"))
    }

    // MARK: - Categories

    func testCategoryClassic() {
        XCTAssertEqual(SynthesisEngineType.subtractive.category, .classic)
        XCTAssertEqual(SynthesisEngineType.fm.category, .classic)
        XCTAssertEqual(SynthesisEngineType.wavetable.category, .classic)
        XCTAssertEqual(SynthesisEngineType.additive.category, .classic)
    }

    func testCategoryTexture() {
        XCTAssertEqual(SynthesisEngineType.granular.category, .texture)
        XCTAssertEqual(SynthesisEngineType.spectral.category, .texture)
    }

    func testCategoryPhysical() {
        XCTAssertEqual(SynthesisEngineType.physicalModeling.category, .physical)
        XCTAssertEqual(SynthesisEngineType.karplusStrong.category, .physical)
        XCTAssertEqual(SynthesisEngineType.waveguide.category, .physical)
        XCTAssertEqual(SynthesisEngineType.modal.category, .physical)
        XCTAssertEqual(SynthesisEngineType.modalBank.category, .physical)
    }

    func testCategorySample() {
        XCTAssertEqual(SynthesisEngineType.sampler.category, .sample)
        XCTAssertEqual(SynthesisEngineType.echoelSampler.category, .sample)
        XCTAssertEqual(SynthesisEngineType.breakbeat.category, .sample)
    }

    func testCategoryOrganic() {
        XCTAssertEqual(SynthesisEngineType.genetic.category, .organic)
        XCTAssertEqual(SynthesisEngineType.organic.category, .organic)
    }

    func testCategoryBioReactive() {
        XCTAssertEqual(SynthesisEngineType.bioReactive.category, .bioReactive)
    }

    func testEveryEngineHasCategory() {
        for engine in SynthesisEngineType.allCases {
            let category = engine.category
            XCTAssertTrue(SynthesisCategory.allCases.contains(category),
                          "\(engine) has invalid category \(category)")
        }
    }

    // MARK: - Real-Time Modulation

    func testSupportsRealTimeModulation() {
        XCTAssertTrue(SynthesisEngineType.subtractive.supportsRealTimeModulation)
        XCTAssertTrue(SynthesisEngineType.fm.supportsRealTimeModulation)
        XCTAssertTrue(SynthesisEngineType.bioReactive.supportsRealTimeModulation)
        XCTAssertFalse(SynthesisEngineType.neural.supportsRealTimeModulation)
    }

    func testOnlyNeuralDoesNotSupportRealTimeModulation() {
        for engine in SynthesisEngineType.allCases {
            if engine == .neural {
                XCTAssertFalse(engine.supportsRealTimeModulation)
            } else {
                XCTAssertTrue(engine.supportsRealTimeModulation,
                              "\(engine) should support real-time modulation")
            }
        }
    }
    ...
    func testCodableRoundTrip() throws {
        for engine in SynthesisEngineType.allCases {
            ...
            let decoded = try JSONDecoder().decode(SynthesisEngineType.self, from: data)
            XCTAssertEqual(engine, decoded, "Codable round-trip failed for \(engine)")
        }
    }

    func testDecodableFromRawString() throws {
        let json = Data("\"ddsp\"".utf8)
        let decoded = try JSONDecoder().decode(SynthesisEngineType.self, from: json)
        XCTAssertEqual(decoded, .ddsp)
    }
}
...
final class SynthesisCategoryTests: XCTestCase {
    ...
    func testAllCasesCount() {
        XCTAssertEqual(SynthesisCategory.allCases.count, 7)
    }

    func testRawValues() {
        XCTAssertEqual(SynthesisCategory.classic.rawValue, "Classic")
        XCTAssertEqual(SynthesisCategory.texture.rawValue, "Texture")
        XCTAssertEqual(SynthesisCategory.physical.rawValue, "Physical")
        XCTAssertEqual(SynthesisCategory.sample.rawValue, "Sample")
        XCTAssertEqual(SynthesisCategory.advanced.rawValue, "Advanced")
        XCTAssertEqual(SynthesisCategory.organic.rawValue, "Organic")
        XCTAssertEqual(SynthesisCategory.bioReactive.rawValue, "Bio-Reactive")
    }

    func testEnginesPropertyReturnsCorrectEngines() {
        let classicEngines = SynthesisCategory.classic.engines
        XCTAssertTrue(classicEngines.contains(.subtractive))
        XCTAssertTrue(classicEngines.contains(.fm))
        XCTAssertTrue(classicEngines.contains(.wavetable))
        XCTAssertTrue(classicEngines.contains(.additive))
        XCTAssertEqual(classicEngines.count, 4)
    }

    func testTextureEngines() {
        let textureEngines = SynthesisCategory.texture.engines
        XCTAssertTrue(textureEngines.contains(.granular))
        XCTAssertTrue(textureEngines.contains(.spectral))
        XCTAssertEqual(textureEngines.count, 2)
    }

    func testEveryCategoryHasAtLeastOneEngine() {
        for category in SynthesisCategory.allCases {
            XCTAssertFalse(category.engines.isEmpty,
                           "\(category) has no engines")
        }
    }

    func testAllEnginesAreCoveredByCategories() {
        let allCategorizedEngines = SynthesisCategory.allCases.flatMap { $0.engines }
        for engine in SynthesisEngineType.allCases {
            XCTAssertTrue(allCategorizedEngines.contains(engine),
                          "\(engine) is not in any category's engines list")
        }
    }
}
