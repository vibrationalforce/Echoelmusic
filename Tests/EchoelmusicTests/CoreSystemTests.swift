...
//
// Tests for core infrastructure: SPSCQueue, CircuitBreaker, NumericExtensions,
// AudioConstants, TuningReference, MusicalNote, RetryPolicy
...
final class CircuitBreakerTests: XCTestCase {
    ...
    func testFailureRecording() async {
        let breaker = CircuitBreaker(name: "test", config: .init(failureThreshold: 5))
    ...
    func testOpensAfterThreshold() async {
        let config = CircuitBreaker.Configuration(failureThreshold: 2, recoveryTimeout: 60)
        let breaker = CircuitBreaker(name: "test", config: config)
    ...
    func testOpenCircuitRejectsRequests() async {
        let config = CircuitBreaker.Configuration(failureThreshold: 1, recoveryTimeout: 300)
        let breaker = CircuitBreaker(name: "test", config: config)

        // Trip breaker
        do {
            _ = try await breaker.execute { throw NSError(domain: "test", code: 1) }
        } catch {}

        // Next request should be rejected
        do {
            _ = try await breaker.execute { return 42 }
            XCTFail("Should have thrown circuitOpen")
        } catch let error as CircuitBreaker.CircuitBreakerError {
    ...
    func testForceOpenAndClose() async {
        let breaker = CircuitBreaker(name: "test")
        ...
        let closedState = await breaker.state
    ...
    func testConfigurationPresets() {
        let def = CircuitBreaker.Configuration.default
        XCTAssertEqual(def.failureThreshold, 5)

        let aggressive = CircuitBreaker.Configuration.aggressive
        XCTAssertEqual(aggressive.failureThreshold, 3)
        XCTAssertEqual(aggressive.recoveryTimeout, 10)

        let relaxed = CircuitBreaker.Configuration.relaxed
        XCTAssertEqual(relaxed.failureThreshold, 10)

        let streaming = CircuitBreaker.Configuration.streaming
        XCTAssertFalse(streaming.useExponentialBackoff)
    }
}
...
final class RetryPolicyTests: XCTestCase {
    ...
    func testDelayCappedAtMax() {
        ...
        // 1.0 * 10^3 = 1000, capped at 5
        XCTAssertEqual(policy.delay(for: 3), 5.0, accuracy: 0.01)
    }
