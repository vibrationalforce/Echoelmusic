...
# Generate Xcode project using xcodegen if available, otherwise use swift package
...
if [ $? -eq 0 ]; then
    ...
    if command -v xcodebuild &> /dev/null; then
        ...
        open Echoelmusic.xcodeproj 2>/dev/null || open Echoelmusic.xcworkspace 2>/dev/null || echo "   Please open Echoelmusic.xcodeproj manually"
        echo ""
        ...
        echo -e "   ${GREEN}d)${NC} Bundle ID: ${CYAN}com.echoelmusic.app${NC}"
        echo ""
        echo -e "   ${GREEN}e)${NC} Connect your iPhone via USB"
        echo -e "   ${GREEN}f)${NC} Product → Destination → Your iPhone"
        ...
        echo -e "   ${GREEN}•${NC} Grant HealthKit permission (for HRV biofeedback)"
        echo -e "   ${GREEN}•${NC} Grant camera permission (for face tracking)"
        echo ""
