...
class NodeGraph: ObservableObject {
    ...
    @Published var nodes: [EchoelNode] = []
    ...
    private let audioQueue = DispatchQueue(
        label: "com.echoel.nodegraph.audio",
        qos: .userInteractive
    )
    ...
    func addNode(_ node: EchoelNode) {
        nodes.append(node)
    ...
    /// Get node by ID
    func node(withID id: UUID) -> EchoelNode? {
    ...
    func savePreset(name: String) -> NodeGraphPreset {
        let nodeManifests = nodes.map { node in
            (node as? BaseEchoelNode)?.createManifest()
        }.compactMap { $0 }
