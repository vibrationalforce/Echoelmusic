...
| G10a ⭐ #1349 (NUR gospelChoir) | Gospel & Spiritual | gospelChoir | **Eins von drei**, und die Aufteilung ist gemessen: `overtoneDrone` und `lowBreathDrone` tragen JE ZWEI unabhängige Blocker — beide nennen `just-major` (§5-2, unbeantwortet) UND beide nennen `pedalDrone`, einen **`PadGrammar`-Fall, den es nicht gibt** (vier ausgeliefert, und `MusicStyle.padGrammar` ist für JEDES Genre `nil`, also wäre der erste Arm das Debüt eines Mechanismus). Keiner davon berührt `gospelChoir`. ⭐ **Diese Scheibe debütiert eine RUBRIK**: `.chant` stand mit einem ⛔-Block in `Category`, der seine eigene Freigabebedingung selbst nannte — ein Fall kommt NUR zusammen mit seiner Tür, und `.chant` komme in der Scheibe, die sein erstes Genre schreibt. Das ist diese; der Block ist ZURÜCKGENOMMEN, nicht gelöscht, weil die REGEL lebt (`GenreSubcategoryTests` Anspruch 4). Neu dazu: Regal `.gospelSpiritual` „Gospel & Spiritual", zwischen `.classicalRomantic` und `.europeanFolk` deklariert, weil Anspruch 6 verlangt, dass die Regale einer Rubrik zusammenhängen. ⚠️ **DREI gemessene Abweichungen vom Katalog, alle gegen `soulBallad`** — der nächste Nachbar teilt `major`, `[0,2,4,6]`, `.backbeat` UND `padOctave 4`, unterscheidet sich auf dem schwächeren Identitätsschlüssel also in GENAU EINER Achse: (1) Tempo **88…112@96** statt 72…100@84 — das Katalogfenster liegt INNERHALB `soulBallad`s 64…86, das neue ist davon disjunkt; (2) **`drivingEighths`** statt `offbeatEighths`, was `soulBallad`s eigene Figur ist (achter Besitzer); (3) Lead **„Warm Strings"** statt Choir Vox — ARITHMETIK, nicht Geschmack: Choir Vox stand schon auf 7 bei Decke 7. ⭐ **Zwei Fremdbefunde, beide durch Messen statt Erinnern:** `MusicStyle.Category.title` hat **NULL Produktions-Leser** (der Genre-Picker rendert nur `Subcategory.title`) — die Rubrik-Ebene ist heute reine Ablage, also ist der unvollständige Titel „Chant, Choir & Drone" eine Ablage-Schuld und keine ausgelieferte Über-Behauptung; und `BassGrammar`s `balkanModal`-Kommentar sagte „SIXTH owner" und war der SIEBTE (ein von der Nachbarzeile geerbtes Ordinal, korrigiert). Gemessen nach dem Schnitt: **52 Genres, 35 angeboten, 71 Patches**, alle sechs Lead-Namen jetzt auf 7 bei Decke 7 — das nächste lead-tragende Genre hebt die Decke auf ceil(43/6)=8, also frei. Wächter `GenreBatchTenATests` |
| G10b (OFFEN) | Drone & Overtone | overtoneDrone, lowBreathDrone | BLOCKIERT auf §5-2 (`just-major`) **und** auf dem `PadGrammar.pedalDrone`-Fall. Beide Blocker sind eigene Scheiben; erst danach diese Zeile |
...
| G11b ⭐ #1294 (Figur) | — | *keine* | **Die Figur zuerst, das Genre danach** — und die Aufteilung stammt vom Werkzeug, nicht von mir: `prebatch.py` meldet für `nordicFiddle` als einzigen Blocker „`heldRoot` ist kein ausgelieferter `BassGrammar`-Fall — ihn zu schreiben ist eine eigene Scheibe (ein Enum-Fall, ein `hits`-Arm, ein Map-Arm, ein Wächter)". Ausgeliefert ist damit der Fall **`heldRoot`** (Grundton 12 Schritte, Quinte die letzten 4, **lückenlos** — die einzige Figur der Datei, deren Treffer alle 16 Schritte decken; das IST ihre Identität und nicht die 12/4-Teilung, die bewusst ungepinnt bleibt, #364). AUTHORED AHEAD, wie `rollingSixteenths` durch S1…S5. ⛔ **NICHT `pedalDrone` genannt, obwohl die Katalogzeile das in der BASS-Spalte schreibt:** derselbe Katalog vergibt `pedalDrone` an einen geplanten **`PadGrammar`**-Fall, in der PAD-Spalte eines Dutzends Drone-Genres (§3, Zeile 470). Eine Schreibweise für zwei Enums ist die #416-Form im Wartezustand; wer den zweiten schreibt, erbt die Verwechslung. **Bass = Bordun, Pad = Pedal.** ⚠️ Und die Katalogzeile sagt „droning double-stop fifths" — ein DOPPELGRIFF ist nicht ausdrückbar: `Hit.fifth` wählt die Quinte ANSTELLE des Grundtons, nie daneben. Die ehrliche Näherung ist Grundton-dann-Quinte ohne Lücke, und der Doc-Kommentar sagt genau das, statt das Intervall zu behaupten |
| G11c ⭐ #1295 (NUR nordicFiddle) | European Folk | nordicFiddle | `nordicFiddle` nimmt jetzt `heldRoot` (Wächter-Liste `authoredAhead` geht an dem Tag rot, an dem es einen Besitzer bekommt — das ist Absicht). `balkanModal` bringt ZWEI unabhängige Risiken mit und gehört deshalb nicht in dieselbe Scheibe: es nennt `additive332`, einen **`PadGrammar`-Fall, den es nicht gibt** (vier sind ausgeliefert), und es ist ein ZWEITES `hungarianMinor`-Genre — `blackMetal`s Doc (#1288) behauptet „von keinem anderen Genre benutzt", also ist das eine Rücknahme in der #1290/#1286-Familie, keine Ergänzung. Vorher messen: `python3 scripts/genre-prebatch.py` mit beiden Kandidaten. ⭐ **AUSGEFÜHRT 2026-09-16 — und die Zeile hat sich selbst geteilt:** sie nennt zwei Genres und begruendet im selben Satz, warum `balkanModal` nicht in dieselbe Scheibe gehoert. Beides zusammen auszuliefern haette eine RUECKNAHME (`blackMetal`s „von keinem anderen Genre benutzt") in einem Feature versteckt. Ausgeliefert ist `nordicFiddle` allein: `heldRoot` hat seinen ersten Besitzer, `authoredAhead` ist LEER, zwei neue Patches (63 „Sympathetic Bow", 64 „Drone Sub"), Waechter `GenreBatchElevenCTests`. Gemessen nach dem Schnitt: **50 Genres, 33 angeboten, 67 Patches**, Warm Strings 5→6 unter der Decke 7, kein Fingerabdruck-Paar. **`balkanModal` ist als G11d GEBAUT (#1295b, 2026-09-16)** — und BEIDE Risiken sind aufgeloest statt umgangen: (1) `additive332` wurde **nicht gebaut**, weil `PadGrammar.tresilloChops` bereits 3+3+2 IST (Phasen 0·3·6, dann 8·11·14) — *aksak* ist der Balkan-Name derselben Zelle, und eine zweite Implementierung waere #416; der Vermerk steht jetzt am Case selbst. Zusaetzlich gemessen und vom Plan nie genannt: `MusicStyle.padGrammar` ist fuer **jedes** Genre `nil`, der erste Arm dort waere also das Debuet eines ganzen Mechanismus — eigene Scheibe, und der Waechter pinnt das `nil`. (2) Die `blackMetal`-Ruecknahme steht **an `blackMetal`s eigenem Arm**, nicht nur am neuen: das ist das Doc, das eine Sitzung liest, wenn sie fragt, ob eine Skala frei ist. Vier Achsen ersetzen das Unique-Argument (Voicing auf DERSELBEN Skala: 0,3,7 gegen 0,7,12 · Progression · Lead-Stimme · Register); der Tempo-Überlapp 120…170 gegen 160…200 ist benannt statt verschwiegen. Gemessen nach dem Schnitt: **51 Genres, 34 angeboten, 69 Patches**, Hollow Reed 6→7 exakt an der Decke, kein Fingerabdruck-Paar. Waechter `GenreBatchElevenDTests` |
...
| G14a+b+c ⭐ #1357 cumbia · #1358 tangoMarcato · #1382 andeanHighland — ALLE DREI GEBAUT, DAS REGAL IST VOLL · nur `gnawaGuembri` offen (andere Rubrik) | Africa · Latin America | koraOstinato, gnawaGuembri, andeanHighland, cumbia, tangoMarcato | ⛔ **`tangoMarcato` IST WIE SKIZZIERT NICHT BAUBAR — es kollidiert mit `slowedGothPop`, das am selben Abend ausgeliefert wurde.** Das Blatt sagt `harmonicMinor` + `[0,2,4,6]` + `padOctave 3`; das ergibt `[0, 3, 7, 11]`, den Moll-Dur-Septakkord, und **Anspruch 2 von `GenreBatchFifteenBTwoTests` pinnt als SWEEP über `allCases`, dass den niemand sonst hat** (#1354). Der Wächter ginge rot, und zwar zu Recht: zwei Genres mit demselben Tonika-Akkord auf derselben Skala sind ein Akkord mit zwei Etiketten. **Auflösung ist eine ENTSCHEIDUNG, keine Zahl** — entweder Tango bekommt eine andere Vierstimmigkeit (sein `[0,3,4]`-Vamp trägt die Farbe ohnehin), oder die Eindeutigkeits-Behauptung von #1354 wird zurückgenommen und BEIDE Docs werden mitgezogen (die #1295b-Form). Nicht still eins von beidem tun. · ⛔ **Und die drei Latin-Zeilen zusammen SPRENGEN die Lead-Decke:** gemessen ergeben sie Tragweite 45→48, Decke bleibt 8, und „Hollow Reed" stünde auf **9** (Tango nimmt es, es steht schon auf 8). Auch „Deep Sub" und „Soft Keys" gingen 7→8, also exakt auf die Decke — nach dieser Scheibe hätten FÜNF der sechs Namen keinen Kopfraum mehr. Die Scheibe muss also geteilt werden, und die Lead-Wahl ist dabei erzwungen, nicht frei. · ⭐ **Gute Nachricht, ebenfalls gemessen:** `andeanHighland`s Viertel-Echo HEBT die schlagzeugfreie Cluster-Ratsche 5→6, verschmilzt also nichts (#1353); `cumbia` ist auf allen sieben Achsen sauber, Fingerabdruck-Sweep leer. **`cumbia` allein ist die nächste baubare Scheibe dieser Rubrik** — **AUSGELIEFERT 2026-09-17 als #1357**, Regal + erstes Genre in EINER Scheibe (G10a/G15b-2-Form). ⚠️ **Die Lead-Zeile dieses Plans war falsch und ist gemessen ersetzt worden: „Soft Keys“ auf `.minor` ergibt FÜNF von sieben Achsen gegen `deepHouse` — exakt die Lage, die #1352 `dubEcho` von diesem Namen weggezwungen hat, gegen denselben Nachbarn.** Gekauft wurde nicht der Lead, sondern die SKALA: `.harmonicMinor` statt `.minor`, was den Wert auf vier senkt UND musikalisch das Richtige ist (auf Stufe 4 klingt die V DUR, die i→V7-Wendung IST Cumbia; auf natürlichem Moll wäre es die moll-v von Deep House und Dub). „Soft Keys“ bleibt damit — als OHR-Entscheidung (Cumbia sonidera ist Orgel- und Keyboard-Musik), nicht als Arithmetik: alle drei Namen mit Kopfraum landen auf vier. Bei vier ist es ein GLEICHSTAND (`deepHouse` und `afroHouse`), nicht ein Nächster — und 25 von 38 Genres des Elternbaums saßen ohnehin auf vier, sechs auf fünf, gemessen über alle 703 angebotenen Paare. Zweite Abweichung: Echo `tape` + gerade Sechzehntel, **erste Verwendung dieses Paares in `GenreFX`** (die zwei Beinahe-Treffer sind besetzt: `digital`+Sechzehntel trägt fünf Arme inklusive `ska`, das denselben Archetyp hat, und `pingPong`+Sechzehntel ist `psyProgHouse`s). Mitgezogen im selben Commit: **zwei Sätze in `slowedGothPop`s Doc** — dessen „wenn ein dritter Arm diese Skala nimmt, ist dieser Satz zu korrigieren“, und eine Aufzählung, die „drei“ sagte, VIER nannte und `blackMetal` darunter führte, obwohl dessen `chordTones` `[0,4,7]` eine leere Quinte `[0,7,12]` ergeben (weder Moll-Tonika noch Dur-V). Gemessen nach dem Schnitt: **56 Genres, 39 angeboten, 79 Patches**, GenreFX-Arme 56 von 56, Soft Keys 7→8 exakt an der Decke, kein Fingerabdruck-Paar. Wächter `GenreBatchFourteenTests` (40 Zusicherungen, §0 39 von 40 transkribiert, 39 grün). ⭐ **`tangoMarcato` IST ENTSCHIEDEN UND ENTBLOCKT (2026-09-17, Aufgabe #33) — und die OPTIONSLISTE OBEN WAR UM EINE OPTION ZU LANG.** Sie bot an: andere Vierstimmigkeit ODER Rücknahme von #1354. **Die erste existiert nicht.** `GenreBatchFourVoicingTests` pinnt das vierstimmige Feld auf GENAU ZWEI Formen (`detroitTechno`s `[0,2,6,8]` und den schlichten `[0,2,4,6]`), und auf `harmonicMinor` IST der schlichte `[0,3,7,11]` — also genau der Akkord, um den es geht. Eine dritte Vierstimmigkeit hätte JENEN Wächter und `detroitTechno`s Doc mit umgeschrieben. **Aufgelöst wird es durch den DREIKLANG `[0,2,4]`** — kein Doc zurückgenommen, kein Wächter umgeschrieben, und die harmonisch-Moll-Farbe trägt die PROGRESSION (Stufe 4 klingt dort DUR), genau der Mechanismus von #1354 und #1357. ⚠️ **EIN DRITTER BLOCKER, den diese Zeile nicht kannte:** die naheliegende Tango-Progression `[0,6,5,4]` — der absteigende Tetrachord, DIE Bandoneon-Linie — ist roster-frei und hätte den Fingerabdruck-Sweep bestanden, aber `GenreBatchThreeVoicingTests` rot gemacht: `upliftingTrance` ist das EINZIGE angebotene Genre mit vier verschiedenen Wurzeln und hält dort die STRIKTE Behauptung, mehr zu besuchen als jedes andere angebotene. Vier Würzeln wären ein GLEICHSTAND. **Gesetz daraus: ein Sweep fängt Gleichheit, ein SUPERLATIV fängt Gleichstand — vor dem Schreiben auch die Superlativ-Pins der nächsten Nachbarn lesen, nicht nur `prebatch.py` fahren.** Gewählt: **`[0,6,4]`** (i→♭VII→V, roster-frei, DREI Wurzeln, Kopf des Abstiegs, Dur-V erhalten). **Und der Lead ist ERZWUNGEN, nicht gewählt:** „Hollow Reed“ stünde auf **9 von 47** bei Decke 8; Kopfraum haben nur „Deep Sub“ und **„Warm Strings“**, und letzteres ist zusätzlich das ehrliche (die Orquesta típica ist Bandoneón UND Streicher). Gemessener Kandidat: `.harmonicMinor` · `.backbeat` · 92…128@112 · swing 0.10 · Warm Strings · `[0,6,4]` · `[0,2,4]` · padOctave 3 · `drivingEighths` · Patches 77/78 · kein Delay — **PRE-BATCH OK**, schlimmster angebotener Nachbar 3 von 7 (`blackMetal`: archetype, lead, bass). **GEBAUT als #1358 am 2026-09-17** — Patches 77 „Marcato Reed“ / 78 „Marcato Sub“, FX-Arm mit `delayEnabled: false` AUSDRÜCKLICH gesetzt (ein Preset muss den Zustand einer Stufe SETTLEN, sonst bleibt ein vom vorigen Charakter eingeschalteter Delay klebrig an), `drivingEighths` als NEUNTER Besitzer (gezählt, nicht geerbt). Wächter `GenreBatchFourteenBTests` (39 Zusicherungen, §0 39/39 grün). Gemessen nach dem Schnitt: **57 Genres, 40 angeboten, 81 Patches**, GenreFX-Arme 57 von 57, fünf der sechs Lead-Namen auf 8 bei Decke 8 — nur noch „Deep Sub“ hat Kopfraum, bis Tragweite 49 die Decke auf 9 hebt. ⚠️ **Weiter blockiert bleibt nur `gnawaGuembri`** (Kultur-Zuschreibung und `pedalDrone`); und die Lead-Decke ist jetzt ENGER: vier der sechs Namen stehen auf 8 bei Decke 8, nach Tango fünf — das nächste lead-tragende Genre muss „Deep Sub“ nehmen, bis Tragweite 49 die Decke auf 9 hebt. ⚠️ `gnawaGuembri` bleibt zusätzlich auf dem Kultur-Zuschreibungsrisiko und auf `pedalDrone` blockiert. ⭐ **`andeanHighland` IST GEBAUT als #1382 am 2026-09-19 — das Latin-America-Regal ist damit VOLL.** Patches 79 „Thin Air Pad“ / 80 „Air Sub“, `sparseSub` als SIEBTER Besitzer (gezählt, nicht geerbt), FX-Arm `tape` + Viertel. **Der Lead war ERZWUNGEN, genau wie diese Zeile es vorhergesagt hatte:** alle sechs Namen standen auf 8 bei Decke 8, nur „Deep Sub“ hatte Kopfraum — also nimmt der Sub-Patch ihn. Die Prognose dieser Zeile („das nächste lead-tragende Genre muss ,Deep Sub` nehmen“) ist eingetreten, ohne dass die Tragweite die Decke gehoben hätte. Gemessen nach dem Schnitt: **58 Genres, 41 angeboten, 83 Patches**, GenreFX-Arme 58 von 58, Fingerabdruck-Sweep leer, schlagzeugfreie Delay-Cluster 5→6 (das Echo HEBT die Ratsche, verschmilzt nichts — exakt die Vorhersage von #1353 oben). Harmonie `[0, 5, 6]` auf `.minor` = Wurzeln 0/8/10, also **i → ♭VI → ♭VII**: Moll-Tonika unter zwei DUR-Akkorden. ⚠️ **Drei Korrekturen am eigenen Entwurf, jede eine Mess-Lehre:** (1) ich las `[0,5,6]` als Halbtöne und schrieb „i → v → ♭VI“ — **Grade sind keine Halbtöne**; (2) ich behauptete, `tape` + Viertel hätte nur `rootsReggae` — gemessen SIEBEN Besitzer, der Unterschied ist das Feedback (0.26); (3) ich nannte `delayWow: 0.06` „den am wenigsten verzerrten der sieben“ — `detroitTechno` schreibt gar kein `delayWow` und nimmt die Default-0.0. **Ein weggelassenes Argument ist trotzdem ein Wert (#431); ein Superlativ über eine Menge muss die DEFAULTS lesen, nicht nur die geschriebenen Argumente.** Wächter `GenreBatchFourteenCTests` (6 Ansprüche, 11 Mutanten je RED auf ihrem Anspruch, Kontrolle GRÜN). ⚠️ Und ein vierter Fehler lag in der TRANSKRIPTION, nicht im Code: mein erster Parser des Helligkeits-Anspruchs las **41 von 83** Patch-Blöcken und meldete sauber — der exakte #1350-Defekt. Repariert, Coverage-Assert ergänzt, bei 83/83 neu gefahren, Anspruch hielt. **Mitgezogen im selben Commit: sieben Kopie-Stellen** (`docs/tools.html`, `docs/brainstorming.html`, `docs/press.html` ×2, `docs/architecture.html`, beide `release_notes.txt`) — eine Genre-Zahl ist eine Behauptung. |
| G15a ⭐ #1350 (NUR loFiHipHop) | Lo-Fi & Hazy (BESTAND) | loFiHipHop | **GEBAUT.** Die erste Scheibe der Epik, die KEINE Taxonomie anlegt — `.loFiHazy` und `.underground` existierten, das Genre stellt sich neben `vaporwave`. ⚠️ **Die Gefahr sind zwei Genres auf ANDEREN Regalen, nicht der Regal-Nachbar.** Über die sieben Identitätsachsen gegen alle 53 Genres gemessen sitzen genau zwei auf dem Maximum von VIER: `dubTechno` (scale, chordTones, leadPatchName, padOctave) und `boomBapHipHop` (beatArchetype, bassGrammar, chordTones, padOctave); `vaporwave` teilt zwei. **DREI gemessene Abweichungen vom Katalog:** (1) **`padOctave: 3`** statt 4 — und die Messung sagt etwas ENGERES als der erste Entwurf behauptete: das Maximum bleibt bei vier, was sich ändert ist die MENGE auf dem Maximum (bei 4: `modalJazz`, `jazz`, `electroFunk`, `detroitTechno`; bei 3: `dubTechno`, `boomBapHipHop`) — halbiert, nicht gesenkt; (2) **Lead „Pluck“** statt „Soft Keys“ — die Decke hatte Platz für alle sechs (bearing 42→43, Decke 7→8), die Arithmetik ZWINGT also nichts, sie VERBIETET genau einen: „Soft Keys“ höbe das Maximum auf FÜNF gegen `boomBapHipHop`; welcher der fünf übrigen ist eine Ohr-Entscheidung und steht als solche im Arm; (3) **FX-Teilung `(.quarter, .triplet)`** statt der Katalog-Punktachtel — `boomBapHipHop` ist SCHON Tape auf punktierter Achtel, dasselbe Paar hätte den beiden Armen EIN Echo mit zwei Etiketten gegeben (die #1349-Lehre: ein grüner Fingerabdruck ist der BODEN, nicht die Frage). Erste Verwendung dieses Paares in `GenreFX`; 0,455–0,556 s über das ganze Fenster, weit unter der 2,0-s-Decke. ⭐ **DREI Fremdbefunde, alle durch Messen:** fünf Zahlen im ausgelieferten `Church Choir`-Doc (#1349) waren falsch (Envelope 2,24 statt 3,10, zwei erfundene Nachbarn, Glacier Pad 15,60 statt 16,50, „sechst-langsamster“ statt elft-) · fünf in diesem Batch' eigenem ersten Entwurf · ein falscher Superlativ in `mixLevels`. **EINE Ursache: ein Wegwerf-Parser, der still 31 von 73 `patch(`-Blöcken las.** ⭐ GESETZ, jetzt am `Church Choir`-Arm: **eine Messung, die ihre eigene ABDECKUNG nicht nennen kann, ist keine Messung** — `parsed N of M`, und keine daraus abgeleitete Zahl wird geschrieben, bis N == M. Gemessen nach dem Schnitt: **53 Genres, 36 angeboten, 73 Patches**; „Pluck“ steht auf 8 bei Decke 8, also genau AUF ihr. Wächter `GenreBatchFifteenATests` (45 Ansprüche) |
| G15b-1 ⭐ #1352 (NUR dubEcho) | **Dub & Echo** (neu) | dubEcho | **GEBAUT.** Debütiert ein REGAL, und das Regal heißt nach dem, was es hält: der Plan sagt „Dub & Drone", ausgeliefert ist **„Dub & Echo"**. Grund, und es ist die #1349-Lehre eine Ebene tiefer: `Category.title` hat null Produktions-Leser (deshalb war „Chant, Choir & Drone" mit einem Gospelchor tragbar), **`Subcategory.title` wird vom Genre-Picker als Abschnittskopf GERENDERT** — ein Regal, das nach einer Drone benannt ist, die es nicht hält, ist eine Über-Behauptung, die ein Spieler liest. `droneMetal` ist ohnehin auf `PadGrammar.pedalDrone` blockiert und gehört neben die anderen Pedal-Drone-Genres, nicht neben Dub. ⚠️ **Nächste zwei sind `rootsReggae` und `deepHouse`, beide auf VIER von sieben Achsen** — gegen Roots dieselben vier (`minor`, `.offbeat`, `offbeatEighths`, `padOctave 3`), und **das Quartett IST Dub**, also absichtlich geteilt. **ZWEI gemessene Abweichungen:** (1) Lead **„Hollow Reed"** statt „Soft Keys" — hier ERZWUNGEN, und von zwei unabhängigen Schranken zugleich: „Soft Keys" ergibt FÜNF von sieben gegen `deepHouse`, „Pluck" steht auf 8 bei Decke 8 und bräche sie, „Deep Sub" ergibt fünf gegen `rootsReggae`; das Rohrblatt ist zusätzlich das ehrliche (Dub kompt auf Orgel/Melodica); (2) **`progression: [0, 6]`** statt `[0, 5]` — i→♭VII ist die Dub-Wendung, und `[0,5]` ist `boomBapHipHop`s eigene. ⭐ **Das `--patch`-Werkzeug aus #1351 hat sofort einen echten Defekt gefangen:** der erste „Dub Sub"-Entwurf teilte DREI von vier Hüllkurvenwerten mit `rootsReggae`s Bass — und der Kommentar, der die Trennung begründete, nannte diesen Patch „Roots Sub", er heißt „Roll Sub". **Gesetz am Arm: den Geschwister-Patch MIT dem Werkzeug prüfen, BEVOR man eine Trennung von ihm behauptet.** Gemessen nach dem Schnitt: **54 Genres, 37 angeboten, 75 Patches**, Schwachschlüssel-Sweep LEER. Wächter `GenreBatchFifteenBOneTests` (46 Ansprüche) |
| G15b-2 ⭐ #1354 GEBAUT | Dark Synth Scenes (neu) | slowedGothPop | **AUSGELIEFERT 2026-09-16.** Regal + erstes Genre in EINER Scheibe (G10a-Form). **ZWEI Abweichungen vom Entwurfsblatt, jede gemessen erkauft:** (1) `progression` `[0, 5, 4]` statt `[0, 1]` — `[0, 1]` ist das meistbesetzte Paar der Datei (sechs Genres, vier davon angeboten, darunter `sciFi`, das einzige mit demselben Archetyp), und sein Punkt ist Stufe 4: auf harmonisch Moll klingt die V DUR `[0,4,7]`, auf natürlichem Moll moll `[0,3,7]`. (2) `chordTones` `[0, 2, 4, 6]` statt `[0, 2, 4]` — die Dreiklang-Fassung teilen zweiundzwanzig Arme; die VIERSTIMMIGE ergibt `[0, 3, 7, 11]`, den **Moll-Dur-Septakkord, den in 55 Genres NIEMAND sonst hat**. ⚠️ Die Form ist nicht die Entscheidung, die SKALA ist es: dieselben vier Stufen tragen acht andere Arme, und auf `.minor` würde daraus der gewöhnliche Mollseptakkord — der Wächter sichert beide Hälften. Echo: `pingPong` + punktiertes Achtel (einziger anderer Arm `synthwave`, nicht angeboten), weil `tape` + halbe Note schon DREI angebotene trägt, `sciFi` darunter. Nächste Nachbarn 3 von 6 Achsen, ein GLEICHSTAND aus `sciFi`, `stillMeditation`, `selfObservation` — alle drei `sustained`, die „Choir Vox"-Überschneidung ist also nominell. Gemessen nach dem Schnitt: **55 Genres, 38 angeboten, 77 Patches**, Choir Vox 7→8 exakt an der Decke, kein Fingerabdruck-Paar, Schwachschlüssel-Sweep leer. Wächter `GenreBatchFifteenBTwoTests` (41 Zusicherungen, §0 41/41 grün) |
| G15b-3 (BLOCKIERT) | — | droneMetal | BLOCKIERT auf `PadGrammar.pedalDrone` (Mechanismus-Debüt, `MusicStyle.padGrammar` ist für JEDES Genre `nil`). ⚠️ Gehört NICHT auf „Dub & Echo" — zusammen mit `overtoneDrone`/`lowBreathDrone` auf ein Drone-Regal, sobald der Mechanismus steht. AnchorFloor +droneMetal |
...
⭐ **UND SEIT G6a GIBT ES DAFÜR EIN WERKZEUG STATT EINER REGEL: `python3 scripts/genre-prebatch.py <kandidaten.json>`.**
...
⛔ **UND DAS WERKZEUG LAG BIS #1320 IM SESSION-SCRATCHPAD, während GENAU DIESE ZEILE es als die
Messung nennt, die jede weitere Scheibe zuerst fahren muss.** Ein Scratchpad stirbt mit seinem
Container: das Rezept dieses Plans war eine Sitzung davon entfernt, unausführbar zu sein, und die
nächste Sitzung wäre zum ERINNERN zurückgekehrt — also genau zu dem, wofür #1286 fünf falsche
Prosa-Behauptungen bezahlt hat. Gefunden wurde es nicht durch Nachdenken, sondern beim VERSUCH,
die G11c-Zeile auszuführen (`python3 scripts/prebatch.py` → `No such file`). **Dasselbe Gesetz wie
„ein Zeiger ist nur so haltbar wie das, worauf er zeigt“, eine Ebene weiter: ein REZEPT ist nur so
haltbar wie das Werkzeug, das es nennt.** Seit #1320 liegt es als `scripts/genre-prebatch.py` im
Repo, mit `mstable2.py` eingefaltet (kein zweiter Scratchpad-Zeiger), repo-relativen Pfaden und
einem `--selftest`, der seine zwei schärfsten Prüfungen an Kandidaten fahren lässt, die AUS DEM
BAUM gebaut sind: ein Klon eines ausgelieferten Fingerabdrucks muss kollidieren, und ein
`BassGrammar`-Fall, den es nicht gibt, muss als ABWESEND benannt werden (#808 — eine Nadel, die
nur auf grünen Eingaben läuft, ist von nichts verifiziert). ⚠️ `$SP/poststate.py` ist NICHT
mitgezogen — es hat dasselbe Problem und ist eine eigene Scheibe; wer es braucht, schreibt es neu.
