...
| A8 | ⭐ **VOID ZURÜCKGENOMMEN 2026-09-20 (#1385)** — das AUv3-Target ist zurück, also ist dieser Posten wieder ausführbar und steht als `FOUNDER_DEVICE_SESSION.md` §2b. ⛔ Die alte VOID-Begründung (#819, „Target am 2026-07-24 entfernt") bleibt als Beleg stehen, dass der Posten 2026-07-24…09-20 **zu Recht** tot war. **AUv3-Register-Reparatur** | Log build 2394: self-probe -3000, raw 101 comps alle Apple | ANSAGE an Founder gegeben | App löschen + TestFlight-Neuinstallation → nächster Log zeigt ownAUv3 true | Geräte-Register korrupt, kein App-Bug |
| A9 | ⭐ **VOID ZURÜCKGENOMMEN 2026-09-20 (#1385)**, aber NUR halb — und die Hälften auseinanderzuhalten ist der ganze Punkt: das Target ist zurück, die App-Group-BRÜCKE in die Extension ist es **nicht**. `EchoelmusicAUv3.entitlements` ist absichtlich leer (das `-3000`-Kapitel), also liest der Plugin-Prozess keine geteilten Vitalwerte und degradiert sauber auf „der Host setzt die Parameter". Ein echter Puls IN GarageBand braucht zuerst, dass die App-ID `com.echoelmusic.app.auv3` App Groups im Portal trägt. ⛔ Alte Begründung bleibt stehen. **App-Group-Puls-Brücke** (AUv3 in fremden Hosts) | Founder-Q „AUv3?" | GEBAUT ✓ (6a568d8, Freshness+5.1.3+10Hz) in v288 | EchoelBodyVibe in GarageBand vom echten Puls gespielt | — |

## OFFEN (nach AKTIV nachrücken)

| A10 | ⭐ **HALB GEMESSEN 2026-09-20 (#1386): in AUM (48 kHz) stimmt die Tonhöhe EXAKT** — FFT über die Tonspur der Founder-Aufnahme liefert **220,15 Hz** gegen den 220-Hz-Default, also +1,2 Cent. ⚠️ Das **widerlegt den Befund nicht, es bestätigt seine Form**: richtig genau dann, wenn die Hostrate der genagelten entspricht. Offen bleibt der 44,1-kHz-Fall (GarageBand iOS). **Der Befund selbst, unverändert: der AUv3 ist hart auf 48 kHz genagelt und folgt dem Host-Format NICHT.** `EchoelmusicAudioUnit` konstruiert `EchoelDDSP(sampleRate: 48000)` und `EchoelCellular(…, sampleRate: 48000)` als `let`, und `EchoelDDSP.sampleRate` ist ein `public let` — nach `init` nicht mehr änderbar. `outputBus.format` wird nirgends beobachtet. ⛔ **DIE RICHTUNG STAND HIER FALSCH, korrigiert 2026-09-21 (#1406), und sie ist genau das, wogegen eine Geräteprobe geprüft würde:** es stand „alles klingt ~8,8 % zu hoch“. Die Rechnung geht andersherum — die Engine berechnet einen Schritt pro Sample unter der Annahme, 48000 Schritte seien eine Sekunde; der Host spielt 44100 Samples pro Sekunde, also dauern 48000 Samples 48000/44100 = 1,088 s und JEDE Frequenz kommt mit 44100/48000 = 0,91875 heraus. **Folge in einem 44,1-kHz-Host (GarageBand iOS ist genau das): alles klingt ~8,1 % ZU TIEF (≈ 1,47 Halbtöne), und jeder LFO/Vibrato/Envelope läuft ~8,8 % zu LANGSAM.** ⚠️ Vereinbar mit der einen Messung, die existiert: die AUM-Aufnahme lief auf 48 kHz — Raten gleich — und maß 220,15 Hz gegen 220 Hz Default. Eine übereinstimmende Rate ist der eine Fall, in dem der Defekt unsichtbar ist; deshalb hat er die erste Gerätesitzung überlebt. Kein Absturz, kein Audio-Thread-Verstoß — aber ein Qualitätsfehler in der wahrscheinlichsten Fremd-Host-Konfiguration, und er sieht aus wie „das Plugin ist kaputt". ⭐ **DIE TIEFERE HÄLFTE IST MIT #1406 (2026-09-21) ERLEDIGT — die drei hart kodierten Sub-Engine-Raten IN `EchoelDDSP` (`EchoelSVFilter`, `EchoelLFO`, `EchoelEntrainment`, je `sampleRate: 48000` als Literal) lesen jetzt `self.sampleRate`.** Das war der Grund, warum eine Reparatur ALLEIN in der Extension den Klang NICHT repariert hätte: ein frisch gebautes `EchoelDDSP(sampleRate: 44100)` hätte Filter, LFO und Entrainment weiter auf 48 kHz laufen lassen. Die Änderung ist auf jedem ausgelieferten Pfad bit-identisch (beide Konstruktionsstellen übergeben 48000) und durch `Tests/CISmoke/TheSubEnginesFollowTheirParentsRateTests.swift` festgenagelt. ⭐ **DIE HÖRBARE HÄLFTE IST MIT #1407 (2026-09-21) ERLEDIGT — `allocateRenderResources()` liest `outputBus.format.sampleRate` und richtet beide Engines danach aus, VOR `noteOn`.** ⛔ **Und die Reparatur, die hier als Plan stand, war die FALSCHE: »`synth`/`texture` zu `var`s machen und NEU BAUEN« ist genau der Zug, den `EchoelDDSP.updateReverbDecay` in der eigenen Datei verbietet** — eine Referenz neu zu setzen, die der Render-Thread dereferenziert, hat ARC gerannt und auf dem Gerät mit EXC_BAD_ACCESS beim ersten Generate abgestürzt. Gebaut ist deshalb die IN-PLACE-Form: `setSampleRate(_:)` auf `EchoelDDSP`, `EchoelCellular` und den drei Sub-Engines; die Objekte, die der Render-Block hält, werden nie getauscht. ⭐ **LEHRE: ein Board-Eintrag, der die Reparatur mitliefert, nennt sie als Vorschlag, nicht als Auftrag** — hier hätte das Abschreiben des Plans den teuersten Absturz dieser Datei reproduziert. ⚠️ **NICHT auf die Haupt-App ausgedehnt, absichtlich**: deren Stimmen speisen `AVAudioSourceNode`s, die 48 kHz DEKLARIEREN, und `AVAudioEngine` wandelt für sie auf die Hardware-Rate — das deklarierte Format IST der Vertrag. Wächter: `Tests/CISmoke/TheAUv3FollowsTheHostSampleRateTests.swift` (Anspruch 8 hält diese Asymmetrie fest). ⚠️ **OFFEN BLEIBT NUR NOCH DIE GERÄTEPROBE**: dass das Plugin in einem 44,1-kHz-Host lädt und in Tonhöhe spielt, kann kein Gate zeigen — `Sources/EchoelmusicAUv3/` ist ein anderes Target und wird vom blockierenden Bündel gar nicht kompiliert. | Founder-Gerätetest §2b sagt, ob es überhaupt lädt — erst danach lohnt die Reparatur | eigene Scheibe + Council | 44,1-kHz-Host spielt in Tonhöhe | `FOUNDER_DEVICE_SESSION.md` §2b |
| A11 | ⭐ **GEMESSEN 2026-09-20 (#1386): rund 20 PROZENTPUNKTE DSP für EINE Instanz.** AUMs Anzeige liest **1 %** ohne Plugin und **19–24 %** mit genau einer Instanz, ohne gespielte Note (Founder-Aufnahme, Build 2595). ⚠️ Das ist AUMs Anzeige bei DESSEN Puffergröße, kein Xcode-Profil — ein Größenordnungs-Befund, kein Messwert gegen das `CLAUDE.md`-Budget (CPU < 30 %, FAIL > 50 %). Er reicht für eine Entscheidung: vier Instanzen wären das Gerät. **Die Ursache, unverändert:** Strukturell pro AUSGABE-Sample: eine 128-Iterations-Glättungsschleife plus bis zu 32 skalare `sin()` in `renderAdditive` — grob 160 Float-Ops und 32 Transzendente pro Sample, komplett un-vektorisiert, neben einem `EchoelDDSP`-Pfad, der vDSP-gebatcht ist. Bei 48 kHz sind das ~1,5 Mio. skalare `sin()` pro Sekunde, ZUSÄTZLICH zu dem, was der Host ohnehin rechnet. `CLAUDE.md`-Budget: CPU < 30 %, FAIL > 50 %. **Hier steht bewusst keine geschätzte Prozentzahl** — eine erfundene Zahl wäre schlimmer als die Lücke. Braucht eine Messung in AUM oder Logic auf dem Zielgerät. Billige Zwischenlösung, falls die Messung schlecht ausfällt: `textureAmount` per Default auf 0. | Gerätemessung (Xcode-Gauge am Plugin im Host) | — | CPU-Gauge im Host bei gespieltem Plugin | — |
| A12 | ⭐ **NEU 2026-09-20 (#1386), aus der Founder-Aufnahme: das Plugin KLINGT, sobald es geladen ist — ohne Note, ohne Transport.** Master-Meter springt beim Instanziieren auf −10 dBFS. Das ist so GEBAUT (`allocateRenderResources()` ruft `synth.noteOn(frequency: baseFreqParam.value)`, und der Render-Block sagt es selbst: *the free-running bio tone armed in allocateRenderResources keeps playing*). In der App ist das richtig — ein Drone-Instrument, das atmet. **In einem fremden Host ist es eine Entscheidung, kein Fehler:** Spur anlegen → sofort Ton. Konventionell schweigt ein AU-Instrument bis zur ersten Note; manche Hosts rufen `allocateRenderResources` auch beim Scannen. Alternative: erst bei der ersten Note oder beim ersten Transport-Start armen, per Parameter umschaltbar. **NICHT einseitig geändert — das ist eine Klangentscheidung des Founders.** | Founder-Aufnahme Build 2595 | Founder entscheidet: Drone-beim-Laden JA/NEIN | Gerät: Spur in AUM anlegen, auf Stille hören | `FOUNDER_DEVICE_SESSION.md` §2b |
| A13 | ⭐ **NEU 2026-09-20 (#1386): der Founder hat EchoelBodyVibe in AUMs Liste MIDI PROCESSORS gesucht.** Dort steht es nicht und KANN nicht stehen — es ist ein `aumu` (Instrument), kein `aumi` (MIDI-Prozessor). **Der billigste Weg dorthin ist kein neues Target:** `AudioUnitViewController` erfüllt bereits `AUAudioUnitFactory`, also lässt sich aus DERSELBEN Extension eine ZWEITE AudioComponent vendieren, verzweigt in `createAudioUnit(with:)` am `componentDescription`. Wirkung: der Körper schickt MIDI in den Host und steuert JEDES Instrument dort, nicht nur unseres. `MIDIOutput`/`UMPEncoder` sind gebaut. ⚠️ Kostet einen zweiten `AudioComponents`-Eintrag in `Resources/EchoelmusicAUv3/Info.plist` und eine Verzweigung; `TheAUv3RegistersAndStaysIsolatedTests` Anspruch 1 prüft die Verschachtelung und muss den zweiten Eintrag mittragen. | Founder-Aufnahme Build 2595 (implizit) + Zwei-Produkt-Vorschlag 2026-09-20 | Council, dann eine Scheibe | Gerät: erscheint es in AUMs MIDI-PROCESSORS-Liste | `Sources/EchoelmusicAUv3/AudioUnitViewController.swift:89` |
...
| O15 | ⭐ **NEU 2026-09-21 (#1406), gefunden beim Fahren eines Wächters statt beim Lesen: `EchoelEntrainment.process` trägt den NICHT-rettenden Wrap, den #1207b aus `EchoelLFO` entfernt hat.** `phase += band.centerFrequency / sampleRate`, dann `if phase >= 1.0 { phase -= 1.0 }` — bei einer Rate 0 wird `phase` `inf`, `inf - 1 == inf`, der Wrap greift nie, `cosf(inf * 2π)` ist NaN und das Sample ist NaN. `EchoelLFO` hat an derselben Stelle `phase = phase.isFinite ? phase - phase.rounded(.down) : 0`. ⚠️ **Heute UNERREICHBAR, und das ist gemessen, nicht angenommen**: `git grep -n "EchoelEntrainment(" -- Sources` liefert GENAU EINE Konstruktionsstelle (`EchoelDDSP.init`), und die übergibt seit #1406 die geklammerte `self.sampleRate` (`max(1, …)`). Also ein latenter Defekt, kein lebender — bewusst NICHT in #1406 mitrepariert, weil das die Scheibe auf zwei weitere Dateien geweitet hätte. ⭐ **ERLEDIGT MIT #1407 (2026-09-21) — und NICHT als eigene Scheibe, sondern als VORBEDINGUNG.** Der Eintrag sagte »heute unerreichbar«; #1407 hat genau das geändert, indem es die Rate schreibbar machte (`setSampleRate` refüsiert ≤ 0 und nicht-endlich, aber bewusst NICHT »klein« — #364). ⛔ **Die Begründung war außerdem in der Zahl falsch, und das kam erst beim FAHREN heraus:** »bei einer Rate 0 wird `phase` `inf`« beschreibt den Einschritt-Fall; der reale Weg ist die AKKUMULATION, und die erste Fassung des Wächters schätzte sie um drei Größenordnungen zu schnell (bei 1e-30 braucht es ~8,5 Millionen Aufrufe, nicht achttausend — die Zusicherung wäre auf dem Elternbaum GRÜN gewesen, also vakuum für genau den Defekt, den sie benennt, #367). Gemessen: bei `sampleRate = 4e-36` ist das Inkrement 1e37 (endlich!) und die Phase wird auf Aufruf **34** nicht-endlich. | Wächter-Fahrt #1406 (kein Founder-Ask) | ERLEDIGT #1407 — `Tests/CISmoke/TheAUv3FollowsTheHostSampleRateTests.swift` Anspruch 5 (Regression) und 6 (Gegengewicht: bei 48 kHz sind beide Formen bit-gleich) | Keine Geräteprobe nötig — reine Werttyp-Mathe, im blockierenden Bundle fahrbar |
...
| AU6 | **Bio-Pfad Cross-Thread-COW-Hazard:** `applyBioReactive→updateSpectralEnvelope` schreibt `harmonicAmplitudes`-Array um (nicht-atomar) auf PolySynthVoice/AUv3-Direktpfad | DSP/EchoelDDSP.swift + PolySynthVoice | #23-Klasse Audio-Stabilität | ✅ **GESCHLOSSEN (nachgemessen 2026-08-30, #886).** Der Dateikopf von `EchoelDDSP.swift` sagt es selbst: BEIDE Besitzer rufen `applyBioReactive` im RENDER-Block (`BioReactiveSynthVoice` und `PolySynthVoice` reihen auf dem Poll in eine SPSC-Queue ein und drainen im Render). Der DRITTE Besitzer — der AUv3-KVO-Poll mit den `BioMirror`-Floats — war mit #121 Slice 1 verschwunden. ⭐ **ER IST AM 2026-09-20 ZURÜCK (#1385), UND DER POSTEN BLEIBT TROTZDEM GESCHLOSSEN — aber aus einem ANDEREN Grund, und das ist der Unterschied, der zählt:** die Schließung hing an „der Besitzer existiert nicht", was eine Tatsache über den Bestand ist und deshalb ablaufen kann; sie hängt jetzt an „der Besitzer hält dieselbe Invariante", was eine Tatsache über das VERHALTEN ist. Gemessen am restaurierten File: `synthRef.applyBioReactive(...)` steht INNERHALB von `internalRenderBlock`, also auf dem Render-Thread — exakt wie die beiden anderen Besitzer. `BioMirror` selbst ist kein Array, sondern vier `Float`s (atomare Breite, veraltete Lesungen möglich, zerrissene nicht), und die 10-Hz-Drosselung dahinter ist render-eigen. **Eine Schließung, die an einer Bestandszahl hängt statt an einer Invariante, ist eine Schließung auf Abruf** — und diese hier lief genau einmal ab, ohne dass jemand rot wurde. ⛔ `git grep -n BioMirror -- Sources` lieferte bis #1385 GENAU EINEN Treffer (den Grabstein-Kommentar); heute sind es mehr, und ein aus dieser Zahl abgeleitetes Urteil wäre falsch herum. Folge, wörtlich im Kopf: der In-Place-Rewrite passiert „on the ONE render thread that also reads it, so there is no longer a cross-thread array race / COW hazard on any path". ⚠️ Die Spaltenangabe `:1287` ist bewusst entfernt — eine Zeilennummer in einer lebenden Datei ist ein Datum, kein Ort |
...
| AU5 | AudioEngine Meter-Props `@ObservationIgnored` (60-Hz-Freeze-Landmine) | Audio/AudioEngine.swift (`masterLevel`/`masterLevelR`) | #11 | offen — **aber die Landmine ist seit #919 BEWACHT, auch ohne den Refactor.** `TheMenuHostReadsNoHotStateTests` leitet die 60-Hz-Menge aus der Timer-Closure ab und verbietet ihr Lesen in allen vier Vorfahren (`EchoelmusicApp`, `WorkspaceView`, `SurfaceHost`, `EchoelStudioView`); dieselbe Prüfung deckt `masterVolume` (Transport-Schritt) mit ab, und die Deklarationen tragen jetzt das Gesetz am Ort. Der `@ObservationIgnored`-Umbau bleibt der eigentliche Posten — ein Wächter verhindert den nächsten Leser, er entfernt die Churn nicht. **NICHT „mechanisch"**, siehe die korrigierte Notiz unter dieser Tabelle (#886). Die zwei Wörter widersprachen der Notiz zwölf Zeilen tiefer, die denselben Posten „riskanter Live-Meter-Self-Poll-Refactor" nennt: ein Slogan, der Arbeit KLEINER macht als sie ist, ist gefährlicher als eine falsche Zahl |

> HIGH-Befunde außerhalb der Top-5 (eigene Zyklen): ⛔ ~~AUv3 Cross-Thread-Race + malloc im
> Render (EchoelmusicAudioUnit.swift:226/391 — deferred Target, #50)~~ **NACHGEMESSEN AM
> RESTAURIERTEN CODE 2026-09-20 (#1385): der Render-Block enthält heute keine Allokation** —
> das einzige `.map` darin ist `Optional.map` auf einem `UnsafePointer`, das nicht allokiert,
> und die Datei dokumentiert die Reparatur selbst (`RenderScratch` hält die EINZIGE Referenz
> je Puffer, `&scratch.pad` schreibt in place statt COW ~32 KB pro Callback zu kopieren). Der
> Befund stammt von VOR dieser Reparatur und wurde nie nachgeführt, weil das Target gelöscht
> wurde — **eine Löschung schließt einen Befund nicht, sie versteckt ihn**, und beim Restore
> kommt er als vermeintlich offen zurück. ⚠️ Die ZEILENNUMMERN `:226/391` waren zusätzlich
> schon damals ein Datum, kein Ort. Gegengeprüft vom `audio-thread-reviewer` im selben Commit, ~~Colab umgeht Egress-Gate
...
|---|-----------|-----------|
| B1 | ⭐ **VOID ZURÜCKGENOMMEN 2026-09-20 (#1385)** — mit Target entscheidet die Probe wieder etwas, und der GarageBand-Workaround in der rechten Spalte ist als Schritt 1 in `FOUNDER_DEVICE_SESSION.md` §2b übernommen. ⛔ Alte Begründung bleibt stehen. **a) Externe AUv3 fehlen in der Liste** | `echoel_diag.log` nach iPhone-Neustart (self-probe-Zeile + scan-Zeilen). Workaround kommuniziert: GarageBand einmal öffnen registriert AUv3 neu. |
