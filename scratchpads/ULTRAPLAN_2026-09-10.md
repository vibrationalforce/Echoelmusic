...
| 10 | `fastlane/metadata`: Keywords en/de + en-GB, Description-Erstzeilen, `colour`→`color`, „Meditativ" raus | Marketing Aktion 3 · Audit `docs-claims-4` | S + C | `fastlane/metadata/**` | ✅ #1256 (`c5f89a7`, Council 2026-09-11): Keywords en/de, Promo, drei Fold-Zeilen, `colour`→`color`, „Meditativ" raus, Externer-Bildschirm-Bullet; BEIDE Descriptions unter 4000 Zeichen gepinnt (`TheStoreFrontLinesSellTheRigTests`). NICHT: en-GB (ASC-Lokalisierung = Founder; Critic strich sequencer/MPE/visualizer), Titel/Untertitel, Voll-Rewrite. `deliver` = Founder-Hand (kein Workflow lädt Metadaten) |
...
| 12 | Richtung 3 schließen (a): `._2_0`-UMP-Virtual-Source neben der `._1_0`-Quelle, Schalter im `midiOutSection` | Council Schritt 3a | S | `Audio/MIDIOutput.swift` + Wächter | ✅ #1253 (`a8d17dc`): `midi.out.ump2`, Quelle „Echoelmusic (MIDI 2.0)", Spiegel nur an die virtuelle Quelle; Gerät offen (NEEDS-FOUNDER-VERIFY) |
...
| 15 | OSC-in Control-Whitelist (`NWListener`, `/echoelmusic/ctrl/*`, `bpm` nur `.studioLocked` über neuen `TempoSource`-Fall `.remoteControl`, Opt-in default AUS, Loopback-Test) | Council Schritt 4 | S + C | `Sync/`, `PatchbayView`, `TempoInvariantTests` | ✅ #1255 (`8d31354`): `OSCReceiver` + Whitelist, `net.osc.in.enabled` default AUS, `.remoteControl`; ABWEICHUNG vom Council-Text: KEIN play/stop (OneStartControlTests pinnt drei Start-Pfade per Founder-Entscheidung — Founder-Frage, LEDGER §U); Gerät offen |
...
| 19 | Anker-Skip-Ratchet (`throw XCTSkip` ohne `fileExists` ≤ heute) + `gh-test-verdict.py:105` Zahl → Befehl | Audit `tests-guards-2` (Widerleger: Migration = #806-Sackgasse, Ratchet ist die kleine Form) | S | `Tests/CISmoke/`, `scripts/` | ✅ #1240 (Ratchet 89, zwei Sprossen, Skript-Befehl) |
