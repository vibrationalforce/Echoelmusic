...
|---|---|
| Read an auto-merge `conflict` as a code conflict on the branch | Read the "Merge to main" STEP LOG. 2026-09-25 it was `fatal: refusing to merge unrelated histories`: the workflow fetched the pushed sha with `--depth=50` on a full checkout, which makes the repo SHALLOW; at 113 commits ahead of main the merge base fell below the cut. Locally `git merge-base --is-ancestor origin/main HEAD` was true — a fast-forward cannot conflict. Fixed (founder-approved) in `979383e1a`, guarded by `TheAutoMergeWaitsForTheGatesTests` claim 5b. |
| Treat a Run Tests exit 65 as #396 because the tail window shows 0 failures | Absence from `tail -200` proves nothing (#807). 2026-09-25 (#6323) the red was a REAL, deterministic guard whose premise flipped (`testTheAUv3TargetIsStillAbsent` vs #1385) — found by checking which guards CAN be red on the tree, and proven by a sibling guard reading the same `#filePath` tree passing in the same window. |
| Trust the needle checkers (moved-needles / dead-needles / count-pins / foreign-needles) to catch a guard broken by ADDED code | They cannot: they look for REMOVED or MISSING text. An `XCTAssertFalse(code.contains("X"))` goes red when a slice ADDS X, and none of them reads it (2026-09-25, path 6: `TrackMix.` added to WorkstationView reddened TheTrackInspectorShowsOnlyWiredControlsTests claim 5). Before committing, grep every `XCTAssertFalse`/`!contains` needle of the guards that name the files you changed, against those files. |
...
| Give the visual a HYSTERESIS (pattern persists as the drive falls) off `CoherenceTrend`, or any direction-of-change memory in the Faraday Dish | Don't. `Core/FaradayDish` explicitly models the **supercritical** Landau onset (√(excess) above a threshold, zero below), and a supercritical bifurcation has NO hysteresis — that belongs to the SUBCRITICAL case. Building it would import a different bifurcation into a model that states its own. Measured 2026-09-08: the shader carries no `trend` uniform at all; `coherenceTrend` reaches AUDIO only (`EchoelDDSP`, via `BioReactiveSynthVoice`). If a future slice wants the trend in the picture it must be labelled a NAMED CHOICE like Aurora's ray spacing, not dressed as physics. |
...
| Ein REPO-WEITER Scanner „welcher tote Typ wird in fremder PROSA noch genannt?" (Typ-Deklarationen sammeln, `\bName\b` über die Kommentare der anderen Dateien) | Nicht bauen. Gemessen 2026-09-19 (#1381): **132 Treffer, die Spitze durchweg falsch.** Die Liste wird von NESTED types beherrscht, deren nackter Name ein gewöhnliches englisches Wort ist — `Advance`, `Design`, `Drop`, `Step`, `Phase`, `Snapshot`, `Frame`, `Window`, `Argument`, `Run`, `Active`, `Layer`, `Direction`, `Result`, `Message`, `Policy`, `Entry`, `Track`, `Analysis` — und auf Platz 1 steht `EchoelmusicApp` mit 36, das lebendigste Symbol des Repos (`@main` REFERENZIERT niemand). Das ist #408 im Großen: die Nadel ist nicht eindeutig, und der Fehler läuft in die ALARMIERENDE Richtung, also liest niemand die Ausgabe zweimal (#665/#937). **Stattdessen:** je Kandidat EINEN unterscheidbaren, voll qualifizierten Namen von Hand messen (`git grep` auf `Typ.member` oder `Typ(`), kommentar-gestrippt, und den TYP messen statt den DATEINAMEN (#1376). So sind #1379 und #1381 entstanden. |
...
| Ein Regex-Sweep über die PROSA-Zählbehauptungen in `Sources/`-Kommentaren („`X` hat zero callers"), um daraus ein `source-pins.py` zu bauen | **Nicht bauen.** Gemessen 2026-09-21 (#1428): die hochpräzise Form (Backtick-Symbol + Zählwort + „caller/call site") liefert **33 Behauptungen, davon 16 als Abweichung markiert — und ALLE SECHZEHN sind Artefakte.** Zwei Ausfallarten, beide unheilbar per Regex: (a) **falsches Subjekt** — das Backtick-Symbol neben dem Zählwort ist nicht das, worüber gezählt wird (`Writes exactly \`count\` samples … callers zero-fill`; „`composeHarmonic`'s parameter of the same name. One caller" zählt die UMGEBENDE Funktion); (b) **Vergangenheitsform** — dieses Repo schreibt Rücknahmen absichtlich als Geschichte auf („`setLatencyMode` HAD zero callers", „`EchoelGranular` went with #1305, so the ONE guarding caller is gone"), und eine erfüllte Rücknahme liest sich wie eine falsche Behauptung. Das ist #491 in Werkzeugform: ein Negativ-Scan über eine Datei, die ihre eigenen Rücknahmen zitiert, trifft die Rücknahmen. **Stattdessen:** `count-pins.py` deckt die Zählpins in `Tests/CISmoke` ab (dort steht die Zahl in einer ASSERTION, nicht in Prosa) — für `Sources/`-Prosa je Kandidat EINEN voll qualifizierten Namen von Hand messen, kommentar-gestrippt. |
| Eine Sweep-Ausgabe durch `| tail -N` schicken | `tail` puffert bis EOF: stirbt der Lauf im Timeout, ist die Ausgabedatei **0 Zeilen** und sieht aus wie „nichts gefunden" (#1425). `python3 -u` direkt in eine Datei, Per-Befehl-Timeout, und eine leere Ausgabe ist NIE ein sauberes Ergebnis. |
...
| Fetch a founder's file from **Google Drive** in this container (curl, or the Drive connector's download) (2026-09-09, #1188) | **Measured: the egress policy denies CONNECT to `drive.google.com` AND `drive.usercontent.google.com` with 403** — both hosts appear by name in `curl -sS "$HTTPS_PROXY/__agentproxy/status"` → `recentRelayFailures`. The Drive MCP connector routes elsewhere and CAN list a folder (`search_files` with `parentId = '<folder id>'` works), but `download_file_content` returns **base64**, so a 436 MB video would be ~582 MB in one tool result — do NOT attempt it. **Do this instead:** have the founder run `autocut proxy` on his Mac (measured 25–110× smaller on his own recordings) and drop the small file straight into the chat, where uploads already land in `/root/.claude/uploads/<session>/`. That path is also the answer to his original "Videos hochladen dauert ewig". |
| Read an ffmpeg **build flag** to decide whether a filter exists (2026-09-09, #1185) | **The flag lies.** The static build here reports `--enable-libfreetype` in `-version` and has **zero** `drawtext` in `-filters`. A build flag is an INTENT; the filter list is the FACT. **Do this instead:** `ffmpeg -hide_banner -filters \| grep -c '<name>'`. ⚠️ The general form is worse than the ffmpeg case: I had already RUN the filter list one command earlier and it did not contain `drawtext` — then overrode that direct measurement with the weaker proxy. **When a direct measurement is already in the transcript, a build flag/config/doc string cannot outrank it.** |
| Conclude a tool is unavailable because it is not on `PATH` (2026-09-09, #1184) | **`command -v ffmpeg` → nothing is TRUE here and "no ffmpeg" is FALSE.** The wheel `imageio-ffmpeg` ships a real binary: `python3 -m pip install --quiet imageio-ffmpeg; FF=$(python3 -c "import imageio_ffmpeg; print(imageio_ffmpeg.get_ffmpeg_exe())")` — and `.claude/skills/watch-clip` has said so in its FIRST LINE since 2026-08-12. Four files carried "the ffmpeg half is untested" on the strength of the PATH check alone. **"Not on PATH" is not "not available"** — a correct measurement can answer a different question than the one asked, and it fails in the reassuring direction. |
| Reuse a sibling checker's helper because the NAME matches what you need (2026-09-09, #1191) | **Read its GATE, not its name.** `foreign-needles.py` imported `dead-needles.stripping_helpers` to answer "does this loader strip comments?" — and its `HELPER_DEF` is a hard allowlist of five names (`codeText\|read\|body\|rawSource\|source`), exactly right for that file's `Sources/`-only charter and blind to every loader the new script actually meets (`text`, `rawFile`, `file`, `codeLines`, `routineText`). The self-test went red on claim 4 and named it. **The failure direction is what matters:** an undetected stripping loader would have produced a FALSE ALARM, and a checker with false alarms is a checker nobody runs (#665). **Do this instead:** reuse the helpers that ask the SAME question in both files (`decode_needle`, `strip_comments`) and re-derive the one that carries the other file's charter. A shared name is not a shared contract. |
| Fix a NaN/range defect at the SINK that shows it (the setter with the bare clamp) and call the boundary closed (2026-09-25, 890837252) | **Walk UP to where the value is DECIDED first.** `PolySynthVoice.setPan` and `TimelineAudioSink.setPan` got the finite rule, and the review showed that it protected nothing: `MultiRollFanout.pan(forSlot:)` and `AudioLanePlayer.clampedPan` clamp BEFORE the sink with the same bare `max(-1, min(1, p))`, so a NaN arrives there already as 1.0. A sink downstream of a clamp cannot see the input the clamp has already converted. Before editing a setter, `git grep` its callers and read what each one passes (f2bef146d). |

## PLAYBOOKS — reliably works here

**§W-BEFEHLS-SWEEP (2026-09-21, #1425–#1427) — drei echte Defekte an einem Nachmittag.** Jeden backtick-zitierten BEFEHL aus den Gesetzesdateien extrahieren und NEU AUSFÜHREN, das Ergebnis neben das Zitat drucken. Read-only per Allowlist der ersten Tokens + Veto-Liste (`>`, `commit`, `checkout`, …). Erste Fläche: `CLAUDE.md` + `.claude/rules/*.md` (**64 befehlsförmige Spannen**). Zweite: `Tests/CISmoke/CLAUDE.md` + `.claude/skills/*/SKILL.md` + `.claude/commands/*.md` (**40**). Ausbeute: eine SUBSTANZIELL falsche Behauptung in der TECH-STACK-Tabelle (#1425), ein Bann-Scan, der in beide Richtungen log (#1426), sechs kommentar-verfälschte Zitate (#1427). ⚠️ **Zwei Fallen, beide bezahlt:** niemals `| tail` (siehe DEAD-ENDS); und ein Befehl aus einer MARKDOWN-TABELLE trägt escapete Pipes (`\|`), die in ERE literal sind — der Sweep meldete dadurch eine Stripper-Migration als fertig, die es nicht ist (richtig gelaufen: 74, nicht 0). **Ein Werkzeug-Artefakt sieht aus wie ein Repo-Befund; der Unterschied kostet einen Lauf von Hand.**
...
| A new media/content tool passes every test and you are about to call it usable (2026-09-09, #1187/#1188) | **Run it on REAL founder material, then LOOK at the output.** Two separate steps, both needed, and neither is a test. (a) `zoom` had 8 selftest claims, 4 drive claims and 7 red mutants — and returned **100 % of the frame** (i.e. did nothing) on all three real iPhone recordings, because a running Echoel session animates the WHOLE screen. Measured concentration: 20.3 / 33.2 / 33.9 % against 100 % synthetic. (b) `proxy` then reported a *great* ratio (25–110× smaller) while its portrait output was **unreadable** — `scale=-2:480` sizes by HEIGHT, giving portrait 220×480 = 106k pixels against landscape's 410k. Only extracting a frame and reading it showed that. **A test proves the ARITHMETIC; real material proves the USE; the eye proves the RESULT. A metric about a picture is not a statement about the picture.** Founder recordings already in this container: `/root/.claude/uploads/<session>/*.mp4`. |
| Mutation-testing an overlay / watermark / "draw something on it" guard (2026-09-09, #1185) | **Assert PRESENCE, not just CHANGE — and judge mutants by EXIT CODE, not by counting `FAIL` lines.** A mutant that scaled the mark to full frame pushed it off-screen via `overlay=W-…`, leaving an empty dark plate, and every claim stayed green: they checked that the plate was dark, in the right corner, and that the original was untouched. **A watermark with no mark passed every test.** The claim that catches it measures CONTRAST INSIDE the plate, inset ~15 % (without the inset the 1 px border and the antialiased edge separate mark-present from mark-absent by only 14 points; with it, 108). Three counting traps hit the same afternoon: (1) an absent guard makes ffmpeg itself abort → exit 1 with **zero** `FAIL` lines; (2) a mutant copied OUTSIDE the repo cannot resolve `repo_root()` from `__file__` and dies before the claims — keep the copy in-tree (`ContentPipeline/Automation/_mutant_tmp.py`, delete after, check `git status`); (3) I measured the border's influence at **1080p** (1 px of 140 → harmless) and cleared it, while the test runs at **480p** (1 px of 62 → dominant). **A measurement at a different SCALE than the thing under test is a different measurement.** |
| Shipping a new repo-checker (2026-09-09, #1191) | **Run it on the REAL tree before you trust the self-test, and treat its first finding as the best test case you will ever get.** `foreign-needles.py --selftest` printed 7 green claims; the first real run reported one finding — and the finding was the CHECKER'S, not the repo's: `TheNeedleCheckerNamesBothErrorDirectionsTests` had RETRACTED an assertion and kept it quoted in a `//` block explaining why, so reading the guard raw parsed a withdrawn line as live. The repair (comment-blank the guard before extracting assertions, the same thing `dead-needles.py` does to the file it SEARCHES) became claim 6b. **Then prove the tool catches the thing it was built for:** a mutant root (temp dir + a symlink to `Tests/CISmoke` + one reworded copy of `CLAUDE.md`) reproduced the #1190 break — control exit 0, mutant exit 1 naming both needles. A checker whose known positive was never driven is a green with no evidence behind it. **And give it a vacuous-pass exit:** zero needles extracted returns 2, not 0 (`.claude/rules/context.md` §2 — a parser that matches nothing is a finding, never a pass). |
| **Golden-Test ≠ Richtigkeit** (2026-09-10, #1210): vier Golden-Tests pinnten `/adm/obj/{n}/position/azimuth` — eine Adressform, die in keiner ADM-OSC-Spec existiert — monatelang grün | Ein Golden-Test beweist, dass ein Formatierer STABIL ist. Für jedes Wire-Format, das ein fremder Standard definiert (ADM-OSC, E1.31, Art-Net, MIDI-UMP), zusätzlich EINEN Wächter gegen die SPEC-TABELLE schreiben (Leaf-Menge, Byte-Offsets), Quelle mit Zeilennummer im Kopf; die Spec beim Schreiben LADEN, nicht aus dem Gedächtnis zitieren (`curl` auf den Upstream-Rohtext, Ergebnis im Scratchpad) |
