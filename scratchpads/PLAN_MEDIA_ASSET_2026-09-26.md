...
| MA4.4 | content digest off-main: SHA-256 streamed in chunks, `Task.detached`, cancellation checked per chunk, one in flight per asset, never at launch; written back through the store | DEFERRED 2026-09-26: the digest needs a hash implementation — CryptoKit is a NEW framework (Council/founder, CLAUDE.md DO NOT), a hand-written SHA-256 is protected-quality code nobody asked for. Founder: "checksum where appropriate". Until then a record REFUTES by duration only; `recordRefusal` already takes a `contentDigest:` so the digest slots in without a signature change. |
| MA4.5 | relink validates against the record (`match`) and rebinds: record id, clip id, region ids, tempo, automation unchanged; one undo step | BUILT `8ecc00be0`: `recordRefusal` (refutes only) → `MediaAssetStore.Rebinding` → `TimelineStore.relinkClipSource` pushes ONE `.clipSource` step carrying clip link + record binding; `rebind(id:toFileName:)` keeps the id. Review repair (MED-1/2): `MediaAssetStore.RelinkIdentity` — the chosen file's OWN record is adopted first; the clip's record moves only while it still describes this clip's missing file; otherwise the link is released. An unlinked clip therefore adopts the file's record (the MA4.3 row's lazy link). |
