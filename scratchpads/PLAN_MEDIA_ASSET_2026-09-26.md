...
| MA4.2 | registry store (App Group, one file, lossy array), `Clip.mediaAssetID` (optional, lossy decode, in the envelope via `clipSlots`); inverts claim 14 of `TheWorkstationImportsAudioTests` and the "no index file" header of `Core/MediaAsset.swift`. BUILT together with the fresh-copy half of MA4.3: `AudioImport.commit(…assets:)` (required, #431) registers the copy's record FIRST, the clip carries its id; the library placement passes `nil` for now | `eeaee9daf` + review repair `bcaeea552` (MED-1: a relink kept the old record's id — now released by the relink, restored by Undo, in the `.clipSource` step) |
| MA4.3 | the library-file paths: Place (orphan) and the MA2 reuse link the existing file's record, or adopt one lazily if it has none; no scan. BUILT as `AudioImport.AssetIdentity` (`.freshCopy` always mints · `.libraryFile` adopts the record bound to the name unless the measurement REFUTES it (`MediaAssetRecord.isContradicted`) · `.unlinked`); a REUSED clip is not rewritten (a pre-MA4.2 clip stays unlinked — lazy link on relink/MA4.5 or a later explicit pass) | `7a38af018` (review running) |
| MA4.4 | content digest off-main: SHA-256 streamed in chunks, `Task.detached`, cancellation checked per chunk, one in flight per asset, never at launch; written back through the store | BUILT `66d37a8c5` (founder 2026-09-26 "MA4.4 APPROVED WITH CRYPTOKIT"; the earlier DEFERRED row is superseded): `Core/MediaContentDigest` — CryptoKit is the implementation, the persisted form is portable text `sha256:<64 hex>`; 1 MiB chunks, cancellation per chunk, detached utility task, `learn` once per record. Computed ONLY after an import landing MINTS a record (Workstation post-import task — since MA4.4c `79a1cccdb` this includes an orphan library file without a record; the "fresh copies only / OPEN GAP" wording further down this row is SUPERSEDED) and in a relink whose record has a digest to compare. `MediaAssetStore.learnDigest` only ADDS. Relink hierarchy A (equal digest → move with id) · B (different → refuse) · C (adopt the file's own record) · D (no proof → new record for the file); a duration never moves a shared record. Guards: claims 9–11 of `TheMediaAssetIsADurableIdentityTests` + journey §8. Review 1: no HIGH, M1 (import hash cancelled by the next import) + L1–L4 → `292d2d4c8`; re-review: no HIGH/MED, two LOW (comment; OPEN GAP: a library file whose record is minted on landing is not hashed — `Landing` lacks adopted-vs-created) → comment-only follow-up `1506446a4`. Compile Check 2964 GREEN on `4735f81e2`; **Build for Testing GREEN (CI/CD 6429)**; Run Tests #396 shape, 167 pass / 0 fail in window, `TheWorkstationJourneySurvivesSaveAndOpenTests.testTheJourneyComesBackWholeAndPlays` OBSERVED PASSING (digest → Save/Open → proven relink → Save/Open); claims 9–11 not in window = execution UNRECORDED. main = `4735f81e2`. **AUTONOMOUS GATES CLOSED / FOUNDER DEVICE ACCEPTANCE PENDING.** |
| MA4.5 | relink validates against the record (`match`) and rebinds: record id, clip id, region ids, tempo, automation unchanged; one undo step | BUILT `8ecc00be0`: `recordRefusal` (refutes only) → `MediaAssetStore.Rebinding` → `TimelineStore.relinkClipSource` pushes ONE `.clipSource` step carrying clip link + record binding; `rebind(id:toFileName:)` keeps the id. Review repair (MED-1/2): `MediaAssetStore.RelinkIdentity` — the chosen file's OWN record is adopted first; the clip's record moves only while it still describes this clip's missing file; otherwise the link is released. An unlinked clip therefore adopts the file's record (the MA4.3 row's lazy link). ⛔ SUPERSEDED IN PART by MA4.4: "move while the record still names this clip's missing file" now also requires EQUAL SHA-256; without a digest nothing shared moves. |
| MA4.6 | proof: Save/Reopen keeps the link; missing → relink keeps every creative id; legacy clips without an id keep working by `mediaRef` | BUILT `31aa7f3a7`: `TheWorkstationJourneySurvivesSaveAndOpenTests` — import links, Save/Open keeps the link, a registry read fresh from disk resolves it, relink keeps song/clip/record ids and persists the move, Undo restores grid AND binding. Legacy (nil link) path: claim 9 of `TheMediaAssetIsADurableIdentityTests`. |
...
### MA4.4c — handoff contract (orchestrator, 2026-09-26 16:40Z)

- **TASK:** a record the landing MINTS gets its SHA-256; an adopted record is never hashed on landing.
- **GOAL:** close the re-review gap of `292d2d4c8` — an orphan library file whose record `establishIdentity`
  mints on this landing is skipped today, so that asset can never prove a relink (founder §3: create
  record → SHA-256).
- **CURRENT HEAD:** `4735f81e2` + docs; main = `4735f81e2`.
- **CANONICAL OWNERS:** `AudioImport.establishIdentity` (the one place that knows minted vs adopted),
  `AudioImport.Landing`, `MediaPlacement.Placed`, `WorkstationView.learnContentDigest`.
- **ALLOWED FILE AREA:** `Sequencer/AudioImport.swift`, `Sequencer/MediaPlacement.swift`,
  `Studio/WorkstationView.swift`; guards `TheMediaAssetIsADurableIdentityTests`,
  `TheImportReusesAnIdenticalLibraryFileTests`.
- **MUST PRESERVE:** record before clip before region; a reused clip is not rewritten; `learn` hashes a
  record once; no launch/scan/periodic hashing; `reusedLibraryFile` keeps its meaning.
- **DO NOT CHANGE:** `MediaBrowserView` (its Place door is MA4.4d), `MediaRelink`, `MediaAssetStore`.
- **ACCEPTANCE:** fresh copy → minted; orphan library file without record → minted; library file whose
  record is adopted → not minted; reused clip → not minted; `.unlinked` → not minted; refused register
  → not minted; the Workstation hashes iff minted. New flags are required init params (#431).
- **REQUIRED REVIEWERS:** one independent read-only reviewer. **REQUIRED TESTS:** the two guards above
  (transcription) + Compile Check + Build for Testing.
- **STATE:** BUILT `79a1cccdb`; reviewed (no HIGH; MED = no guard on the import door's orphan `Landing`
  with a registry → repaired `fe5a715c6`, lands twice: mint then adopt; 2 LOW stale prose repaired;
