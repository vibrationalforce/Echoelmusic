#!/bin/bash
#
# deploy.sh - Deploy ECHOEL to iPhone (via Xcode)
...
# Banner
echo -e "${MAGENTA}╔═══════════════════════════════════════╗${NC}"
echo -e "${MAGENTA}║  📱 ECHOEL - Deploy to iPhone         ║${NC}"
echo -e "${MAGENTA}╚═══════════════════════════════════════╝${NC}"
echo ""
...
if [ $? -eq 0 ]; then
    ...
    if command -v xcodebuild &> /dev/null; then
        ...
        open Echoel.xcodeproj
        echo ""
        echo -e "${YELLOW}2. In Xcode, do the following:${NC}"
        echo ""
        echo -e "   ${GREEN}a)${NC} Select the 'Echoel' target in the project navigator"
        echo -e "   ${GREEN}b)${NC} Go to 'Signing & Capabilities' tab"
        echo -e "   ${GREEN}c)${NC} Select your Team (Apple ID)"
        echo -e "   ${GREEN}d)${NC} Bundle ID: ${CYAN}com.vibrationalforce.echoel${NC}"
        echo ""
