...
# 🌊 ECHOEL Development Helper Script
...
function print_header() {
    echo -e "${BLUE}🌊 ECHOEL Dev Tool${NC}"
    echo -e "${BLUE}===============${NC}\n"
}

function show_help() {
    print_header
    echo "Usage: ./echoel-dev.sh [command]"
    echo ""
    echo "Commands:"
...
function build_project() {
    echo -e "${YELLOW}🔨 Building ECHOEL...${NC}"
    swift build
...
function create_feature_branch() {
    if [ -z "$1" ]; then
        echo -e "${YELLOW}Usage: ./echoel-dev.sh feature <feature-name>${NC}"
        exit 1
    fi
