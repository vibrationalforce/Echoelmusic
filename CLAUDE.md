...
- **Vokal-Kette (Founder-Ask 2026-08-20) — WAS AUF SEINER STIMME LIEGT UND WAS NICHT (#700/#701):** ⭐ **Die EINE Tür ist seit #1247 zurück** (Founder 2026-09-11, „Audio Input sauber aufsetzen"): `masterDoorButton` „Audio input" im Master-Panel setzt `showInput`; #1024 hatte alle drei Türen genommen. Bedienbar ja, **am Gerät UNBESTÄTIGT** — die `isInputConnToConverter`-Absturzfamilie ist nicht geschlossen; Scheiben in `scratchpads/PLAN_AUDIO_INPUT_2026-09-11.md`. **Monitoring + Autotune sitzen auf dem Monitorpfad** (`input → notchEQ → voiceTunePitch → monitorMixer`; die Tune-Stufe ist seit #858 FEST verdrahtet und bei AUS bypassed — ⛔ **der Live-Umbau dieser Stufe ist eine SACKGASSE: er starb an fünf Geräte-Logs, nicht erneut versuchen**). **Harmonizer auf seiner Stimme SCHALTBAR seit #841 · Granular auf seiner Stimme SCHALTBAR seit #849** — beide default AUS, beide im SELBEN Input-Sheet (die Sheet-Kette wächst nicht), und EIN gemeinsamer `pushVoicePreset()` trägt BEIDE Stufen, damit keine Tür die andere zurücksetzt. ⭐ Die mic-eigene `EchoelFXChain` sitzt auf dem Monitor-Insert (`MonitorInsertAU`) und ist **NEUTRAL: alle 15 Stufen explizit AUS**, Ausgang bit-exakt gleich dem Eingang — die Werks-Defaults Saturation/Chorus/Limiter=AN sind Synth-Bus-Tuning und werden deshalb aktiv ausgeschaltet. Bis der Sänger schaltet, bleibt alles bit-neutral; die KLINGENDEN Synth-Instanzen bearbeiten weiter nur die erzeugte MUSIK. Nach AUSSEN ist nichts über-behauptet — Store-Text, `ContentPipeline/CLAIMS.md` und `EchoelFXView`s Kopf sind zeilenweise geprüft. **KEIN Voice clone** (Founder 2026-08-25, wörtlich „latenzfrei… Harmonizer und Granular… ressourcenschonend“; Frage geschlossen); V0 bleibt die HÖR-Bestätigung, hält den Bau aber nicht an. Wächter: `Tests/CISmoke/TheVocalChainStopsAtTheAutotuneTests.swift` (#364: er verbietet V1b NICHT). ⛔ **Die BAU-Geschichte dieser Zeile — der V1a→V1b-3-Leiter-Stand mit seinen Geräteproben, die fünf `EchoelFXChain(`-Stellen, die #839/#841-Montage-Details und die zwei Rücknahmen im Wächter-Kopf — liegt in `memory/LEDGER_COUNTS.md` §T.**
...
· **`TimelineAutomationRow` = GELÖSCHT (#473, 2026-08-07).** Der Eintrag stand hier als türlos-aber-nicht-löschbar, weil `TimelineAutomationRowMath` in DERSELBEN Datei lebte und `Core/TimelineStore.swift` es ruft — ein „lösch die türlose Datei"-Aufräumen hätte den Store gebrochen. **#472 hat den Kern herausgehoben, #473 hat die Ansicht gelöscht**: `struct TimelineAutomationRow: View` plus `TimelineAutomationTargetOption` und `TimelineAutomationHeadCell`, alle drei mit null externen Verweisen. Die Datei ist weg; `Sequencer/TimelineAutomationRowMath.swift` bleibt und ist unverändert.
  ...
  · **`FaceExpressionBioPublisher` — ACHTER Eintrag (#1002), eine ganze EINGABE-MODALITÄT — ⭐ ERLEDIGT mit #1257 (Founder-Prompt „Kamera als Instrument-Eingang", 2026-09-11):** `git grep -n "FaceExpressionBioPublisher(" -- Sources` → **1** (`EchoelmusicApp`), Tür = vierter Eintrag des Quellen-Dropdowns `BioSourceOption.face` („Play with your face — front camera, no pulse"), nur angeboten, wo `isSupported` (TrueDepth) — das Flag `cameraExpression` wurde NIE der Hebel. `ModSource.faceSmile/.faceBrow/.faceJaw.hasProducer` = true → Smile/Brow/Jaw sind FX-Bio-Carrier. Puls bleibt auf einem Face-Take unmessbar (Konsumenten halten/neutral). Die EU-AI-Act-Rahmung (AUSDRUCK als Steuersignal, NIE Emotion) und der verbreiterte `NSCameraUsageDescription`-Satz (beide Linsen) reisen mit. **Gerät unbestätigt.** Plan: `scratchpads/PLAN_KAMERA_EINGANG_2026-09-11.md` (K2–K7). Wächter: `Tests/CISmoke/TheFaceSourceHasADoorTests.swift`.
