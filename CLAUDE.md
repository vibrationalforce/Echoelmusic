...
· **`TimelineAutomationRow` = GELÖSCHT (#473, 2026-08-07).** Der Eintrag stand hier als türlos-aber-nicht-löschbar, weil `TimelineAutomationRowMath` in DERSELBEN Datei lebte und `Core/TimelineStore.swift` es ruft — ein „lösch die türlose Datei"-Aufräumen hätte den Store gebrochen. **#472 hat den Kern herausgehoben, #473 hat die Ansicht gelöscht**: `struct TimelineAutomationRow: View` plus `TimelineAutomationTargetOption` und `TimelineAutomationHeadCell`, alle drei mit null externen Verweisen. Die Datei ist weg; `Sequencer/TimelineAutomationRowMath.swift` bleibt und ist unverändert.
    ...
    ⭐ **Seit Phase 3 / Automation A1 ist Song-Automation wieder erreichbar:** `SongAutomationEditor` (Workstation, Schalter „Automation" am gewählten POLY-Rack-Track) zeichnet je Parameter eine Kurve (A2: Menü = Projektion von `automatableBases`, Schlüssel `track.<id>.<base>`) über den EINEN Schreiber `TimelineStore.setSongAutomation` — ein Undo-Schritt (`.automation`). Die älteren Per-Punkt-Mutatoren bleiben aufruferlos (sie schreiben keinen Undo). ⚠️ Der Echoel-Track hat KEINE Zeile: der Per-Track-Schlüssel löst dort nicht auf, der globale erreicht nur die Harmonie-Stimme — Founder-Frage. Die globale `AutomationPlayer.lanes`-Schicht hat weiter keinen Schreiber.
