...
⛔ **DIE ENTSCHEIDUNG (v1.0 = iPhone) STEHT — ihre BEGRÜNDUNG ist Sequenzierung und der fehlende Sensor auf iPad, NICHT ein Verzicht aufs Ökosystem.** Diese Zeile hat sich am 2026-07-31 zweimal an einem Tag geirrt: erst behauptete sie iPhone-only, während `project.yml` an allen vier iOS-Targets auch an iPad auslieferte; dann las sie sich als **Ausschluss** von iPad/Vision, genau falsch herum. ⭐ **GESETZ, das eine künftige iPad-Rückkehr braucht, weil sie aus dieser Zeile ihren Aufwand schätzt: Wiederanschalten ist NICHT „eine Zeile" — es sind vier Einstellungen plus der Wächter (`Tests/CISmoke/DeviceFamilyIsPhoneOnlyTests.swift`) plus zwei Prosa-Blöcke.** Der eingängigere Satz war der falsche; hier war nicht eine Zahl veraltet, sondern eine Behauptung wurde nie geprüft, weil sie gut klang und niemandem wehtat. **Ein Slogan, der Arbeit KLEINER macht als sie ist, ist gefährlicher als eine falsche Zahl.** ⛔ **Die PROVENIENZ liegt in `memory/LEDGER_COUNTS.md` §S** — beide Fassungen des 07-31, der Slogan in vier Dateien, und der Grund für die Verschiebung: die Geschichte stand hier in DREI Blöcken, von denen zwei denselben 209-B-Satz wörtlich trugen und der dritte mit „Dieser Satz" auf ein Antezedens zeigte, das die beiden anderen verdeckt hatten.
